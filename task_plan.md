...
- [x] TDD：扩展模块布局架构测试，要求 run orchestration helper 不再散落在 `coding_orchestration/` 包根，并运行 run orchestration 相邻测试。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动 helper 并更新相对 import / 文档路径。
- [x] 验证：运行 focused GREEN、run orchestration 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 models contract 不再散落在 `coding_orchestration/` 包根，并运行既有 state/model/run service 相邻测试。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 architecture guard 边界扫描路径 / 文档路径。
- [x] 验证：运行 focused GREEN、state/run/model 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 295：State machine contract 子包治理
- [x] 定域：只收拢 `state_machine.py` 到 `coding_orchestration/state_machine/machine.py`，通过 `coding_orchestration/state_machine/__init__.py` 保留 `from coding_orchestration.state_machine import TaskStateMachine, InvalidTransition` 稳定导出；保持任务状态转换、runner/source 状态映射和异常语义不变；不迁 `models.py`、RunService、run status transition service、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 state machine contract 不再散落在 `coding_orchestration/` 包根，并运行既有 state machine / run status transition 相邻测试。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新相对 import / 文档路径。
- [x] 验证：运行 focused GREEN、state/run status 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 config contract 不再散落在 `coding_orchestration/` 包根，并运行既有 config contract 测试。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新文档路径。
- [x] 验证：运行 focused GREEN、config/docs 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 Hermes CLI registration 不再散落在 `coding_orchestration/` 包根，并保留 `coding_orchestration.cli` 稳定导出。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、CLI/plugin/docs 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 290：Plugin tool registration 子包治理
- [x] 定域：只收拢 `plugin_tools.py` 到 `coding_orchestration/tools/plugin_tools.py`，保持 Hermes native tool host 注册、ToolSpec schema 包装、operation dispatcher handler 绑定和 legacy register_tool fallback 行为不变；不迁 CLI、orchestrator tool façade、ToolSpec/dispatcher 纯合同、WorkItem/Task service、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 plugin tool registration 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、tool/plugin/CLI 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 status policy 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、status policy / run service / reconcile 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 policy/guard helper 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、policy/diff guard/run service 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 285：Tool contract/dispatcher 子包治理
- [x] 定域：只收拢 `tool_specs.py` 和 `tool_operation_dispatcher.py` 到 `coding_orchestration/tools/`，保持 Hermes native tool 规格合同、operation_id 分发和 plugin_tools host 注册行为不变；不迁 `plugin_tools.py`、CLI/Gateway diagnostic host shell、orchestrator tool façade、WorkItem/Task service、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 tool contract/dispatcher 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / patch target / 文档路径。
- [x] 验证：运行 focused GREEN、tool/CLI/plugin 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 command rule helper 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、command catalog/rewrite/Gateway 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求这两个 service 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、service/Gateway/status/merge-test 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 279：Project domain 子包治理
- [x] 定域：只收拢 `project_*.py` 领域模块到 `coding_orchestration/project/`，保持项目识别、profile catalog、知识初始化/扫描/文档构造、Feishu Project intake 规则和 work item binding identity 行为不变；不迁 ledger/storage schema、Gateway/command host shell、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 project domain 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、project/domain/Gateway/run flow 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 run reconcile writeback service 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、reconcile/status/run flow 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 276：Run completion writeback service 子包治理
- [x] 定域：只收拢 `run_completion_writeback_service.py` 到 `coding_orchestration/run/services/`，保持 fresh completed run 的 completion projection、stale observation、状态 transition、ledger/session/summary/project writeback 和 final result payload 协调行为不变；不迁 active reconcile、run manifest、run orchestration projection、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 run completion writeback service 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、completion/reconcile/run flow 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 275：Source resolver 子包治理
- [x] 定域：只收拢 `source_resolver.py` 到 `coding_orchestration/source/`，保持 `SourceResolver.resolve_source_result()` / legacy `resolve_source()`、Lark preflight 和 Meegle preflight 行为不变；不迁 Feishu reader、Meegle adapter、source context repair、ledger schema、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 source resolver 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、SourceResolver/Feishu/Meegle/diagnostics/CLI 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 274：Source context repair service 子包治理
- [x] 定域：只收拢 `source_context_repair_service.py` 到 `coding_orchestration/source/`，保持 source context 读取、deferred source pre-run enrichment、既有 task context 修复、source summary 回写和项目上下文回填接线语义不变；不迁 `source_resolver.py`、reader、ledger schema、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 source context repair service 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、source context repair 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 273：Meegle reader adapter 子包治理
- [x] 定域：只收拢 `meegle_reader.py` 到 `coding_orchestration/source/adapters/`，保持 Meegle/飞书 Project work item reader gateway/CLI fallback、payload 归一化和 deferred recovery 语义不变；不迁 `source_resolver.py`、`source_context_repair_service.py`、Feishu reader、ledger schema、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 Meegle reader adapter 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、Meegle/SourceResolver 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 source helper 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、source helper 相邻回归、py_compile、YAML、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 270：Install integration 子包治理
- [x] 定域：只收拢 `install.py` 到 `coding_orchestration/integrations/install/`，保持 install preflight、symlink install、legacy uninstall dry-run/execute 语义不变；不触碰真实 Hermes home、auth/token/.env、本地运行根内容、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 install integration 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 scripts / tests / 文档路径。
- [x] 验证：运行 focused GREEN、install/docs 相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 269：Knowledge adapter integration 子包治理
- [x] 定域：只收拢 `knowledge_adapter.py` 和 `llm_wiki_adapter.py` 到 `coding_orchestration/integrations/knowledge/`，保持本地 LLM Wiki Markdown layout、KnowledgePort adapter 和 run summary writer 语义不变；不迁 project knowledge resolver/initializer/inventory、TaskService、runner/workspace/git、run lifecycle 或本地运行根内容。
- [x] TDD：扩展模块布局架构测试，要求 knowledge adapter 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、knowledge / project resolver / run summary 相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：扩展模块布局架构测试，要求 Hermes runtime adapter 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、Hermes runtime / runner 相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete

### 阶段 266：Run summary writer knowledge integration 子包治理
- [x] 定域：只收拢 `run_summary_writer.py` 到 `coding_orchestration/integrations/knowledge/`，保持 `RunSummaryWriter` 对 `KnowledgePort.write_run_summary()` 的端口委托语义不变；不迁 `knowledge_adapter.py`、`llm_wiki_adapter.py`、LLM Wiki layout、run summary writeback service、runner/workspace/git 或 run lifecycle。
- [x] TDD：扩展模块布局架构测试，要求 run summary writer 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused test 在当前包根文件存在时失败。
- [x] 实现：移动模块并更新 import / 文档路径。
- [x] 验证：运行 focused GREEN、summary writer/knowledge 相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步进度、发现、技术方案、项目地图、组件合同、约定、machine-readable context 和治理计划。
- **状态：** complete
...
- [x] TDD：新增架构测试，要求 5 个 run execution host service 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：迁移 5 个 service 并更新 import。
- [x] 验证：运行 focused GREEN、目标 service 自测、相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete
...
- [x] TDD：新增架构测试，要求 run writeback callback service 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/run/services/`，迁移 run ledger/session/summary writeback service 并更新 import。
- [x] 验证：运行 focused GREEN、run writeback service 自测、completion/reconcile/status 相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete

### 阶段 259：Run projection 子包治理第一切片
- [x] 定域：只收拢 `run_*_projection.py` 到 `coding_orchestration/run/projections/`，保持 run projection 纯 payload/规则行为不变。
- [x] TDD：新增架构测试，要求 run projection 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/run/projections/`，迁移 run projection 模块并更新相对 import。
- [x] 验证：运行 focused GREEN、run projection 自测、run orchestration/writeback/checkpoint 相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete
...
- [x] TDD：新增架构测试，要求剩余 command host shell 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/commands/`，迁移 delivery/project command executor 并更新相对 import。
- [x] 验证：运行 focused GREEN、py_compile、delivery/project/command/Gateway 相邻回归、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete

### 阶段 256：Presenter 模块目录治理第一切片
- [x] 定域：只收拢 `coding_orchestration/*_presenter.py` 到专用子包，保持用户可见文案行为不变。
- [x] TDD：新增架构测试，要求 presenter 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/presenters/`，迁移 presenter 模块并更新相对 import。
- [x] 验证：运行 focused GREEN、py_compile、presenter/command/Gateway 相邻回归、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete

### 阶段 255：Feishu 模块目录治理第一切片
- [x] 定域：只收拢 `coding_orchestration/feishu_*.py` 到专用子包，保持 Feishu 文案、reader 和 Project MCP adapter 行为不变。
- [x] TDD：新增架构测试，要求 Feishu 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/feishu/`，迁移 Feishu 模块并更新相对 import。
- [x] 验证：运行 focused GREEN、py_compile、Feishu/source 相邻回归、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete

### 阶段 254：Coding command executor 目录治理第一切片
- [x] 定域：只收拢 `coding_orchestration/coding_*_command_executor.py` 到专用子包，保持命令 host shell 行为不变。
- [x] TDD：新增架构测试，要求 coding command executor 不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/coding_commands/`，迁移 coding command executor 并更新相对 import。
- [x] 验证：运行 focused GREEN、py_compile、command/Gateway 相邻回归、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete

### 阶段 253：Gateway 模块目录治理第一切片
- [x] 定域：只收拢 `coding_orchestration/gateway_*.py` 到专用子包，保持模块职责和运行行为不变。
- [x] TDD：新增架构测试，要求 Gateway 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/gateway/`，迁移 Gateway 模块并更新相对 import。
- [x] 验证：运行 focused GREEN、py_compile、Gateway 相邻回归、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete

### 阶段 252：包根 façade 文件目录治理第一切片
- [x] 定域：只收拢 `coding_orchestration/orchestrator_*_facade.py` 到专用子包，保持模块职责和运行行为不变。
- [x] TDD：新增架构测试，要求 orchestrator façade 模块不再散落在 `coding_orchestration/` 包根。
- [x] RED：确认新增 focused architecture test 在旧目录布局下失败。
- [x] 实现：创建 `coding_orchestration/orchestrator_facades/`，迁移 façade 模块并更新相对 import。
- [x] 验证：运行 focused GREEN、py_compile、相邻回归、architecture guard、diff check 和 release readiness no-smoke。
- [x] 文档：同步目录约定、项目地图、组件合同、machine-readable context、进度和发现。
- **状态：** complete
...
- [x] 定域：只清理 feedback presenter 私有代理；不触碰 `start_run()`、workspace/git/checkpoint、Gateway route 规则、后台 runner 实现或状态机。
- [x] TDD：扩展 `tests/test_coding_feedback_command_executor.py`，要求 FakeHost 不再提供 `CodingOrchestrator._*feedback*` / `_human_clarification_*` 私有文案代理，executor 直接消费 `feedback_presenter.py`。
- [x] RED：focused tests 先失败于 executor 仍调用 host 私有 feedback wrapper。
- [x] 实现：`coding_feedback_command_executor.py` 直接调用 `feedback_presenter.py`，删除 `CodingOrchestrator` 内 feedback 文案代理和无用 import。
...
- [x] 定域：只清理 merge-test presenter 私有代理；不触碰 `start_run()`、workspace/git/checkpoint、blocked readiness 评估、QA evidence 观测、Gateway route 规则或状态机。
- [x] TDD：扩展 `tests/test_coding_merge_test_command_executor.py` 和 `tests/test_gateway_command_executor.py`，要求 executor 在 host 不提供 merge-test 文案私有代理时直接消费 `merge_test_presenter.py`。
- [x] RED：focused tests 先失败于 executor 仍调用 host 私有 merge-test presenter wrapper。
- [x] 实现：executor 直接调用 `merge_test_presenter.py`，删除 `CodingOrchestrator` 内 merge-test 文案代理和无用 import。
- [x] 验证：运行 focused tests、相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 232：Task 37 Orchestrator 500 行治理第五切片
- [x] 定域：只清理 doctor presenter 私有代理；不触碰 diagnostic executor 的 tool/preflight 调用、Gateway route、runner/workspace/git、run lifecycle 或状态机。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再保留 `_doctor_*` 文案私有代理。
- [x] RED：focused test 先失败于 `orchestrator.py` 仍定义 doctor presenter wrapper。
- [x] 实现：删除 `CodingOrchestrator` 内 doctor presenter 文案代理和无用 import，保留 diagnostic executor / doctor presenter 的真实 owner。
- [x] 验证：运行 focused tests、相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 233：Task 37 Orchestrator 500 行治理第六切片
- [x] 定域：只清理 Gateway rewrite presenter 私有代理；不触碰 LLM rewrite 调用、Gateway route、pending action、task 创建、runner/workspace/git、run lifecycle 或状态机。
...
- [x] 定域：迁出 tool operation façade、Project MCP / work item 小包装和 tool dispatch adapter；不迁 task creation/source helper、Gateway route、普通 command façade、active run reconcile、runner/workspace/git、`start_run()` 或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 `tool_*`、`dispatch_tool_operation()`、`_build_tool_operation_dispatcher()` 和 Project MCP tool wrapper。
- [x] RED：focused architecture test 先失败于 `orchestrator.py` 仍定义 29 个 tool façade 方法。
- [x] 实现：新增 `orchestrator_tool_facade.py` mixin 承接 tool operation host façade；`CodingOrchestrator` 继承 mixin 并删除主文件内对应实现。
- [x] 验证：运行 focused architecture test、tool/CLI/project work item 相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 238：Task 37 Orchestrator 500 行治理第十一切片
- [x] 定域：迁出 diagnostics / dashboard façade，包括 doctor/CLI wrapper、Meegle preflight fallback、dashboard status payload 和 preflight/source-resolve formatter wrapper；不迁 Gateway route、tool operation façade、task creation/source helper、runner/workspace/git、`start_run()` 或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 diagnostics/dashboard façade 方法。
- [x] RED：focused architecture test 先失败于 `orchestrator.py` 仍定义 10 个 diagnostics/dashboard façade 方法。
- [x] 实现：新增 `orchestrator_diagnostics_facade.py` mixin 承接 diagnostics/dashboard façade；`CodingOrchestrator` 继承 mixin 并删除主文件内对应实现。
- [x] 验证：运行 focused architecture test、diagnostics/CLI/dashboard 相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 239：Task 37 Orchestrator 500 行治理第十二切片
- [x] 定域：迁出 Gateway route façade，包括 immediate/custom route dispatch、Coding Mode rewrite wrapper、pending action/rewrite binding wrapper、确认/取消词 wrapper、commands listing route、dedupe/授权/looks-like-task wrapper；不迁 `handle_gateway_event()` 入口、task creation/source helper、active task/project binding 业务、runner/workspace/git、`start_run()` 或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 Gateway façade wrapper。
...
### 阶段 241：Task 37 Orchestrator 500 行治理第十四切片
- [x] 定域：迁出 task/source façade，包括 task create command wrapper、TaskService source/status payload wrapper、source context 读取/修复/enrich wrapper、event/media ledger helper 和 draft knowledge source refs wrapper；不迁 active task/coding mode binding、feedback command host shell、run lifecycle、runner/workspace/git、`start_run()` 或状态机核心。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 task/source façade wrapper。
- [x] RED：focused architecture test 先失败于 `orchestrator.py` 仍定义 33 个 task/source façade wrapper。
- [x] 实现：新增 `orchestrator_task_source_facade.py` mixin 承接 task/source façade；`CodingOrchestrator` 继承 mixin 并删除主文件内对应实现。
- [x] 验证：运行 focused architecture test、task/source/media 相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 242：Task 37 Orchestrator 500 行治理第十五切片
- [x] 定域：迁出 task runtime façade，包括 Kanban/status host wrapper、task list/status event wrapper、feedback active-task wrapper、active task/coding mode binding wrapper、task lifecycle wrapper、人工决策记录和 RunService gate wrapper；不迁 `start_run()` 主体、active run reconcile 主体、runner/workspace/git 或 run lifecycle 核心。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 task runtime façade wrapper。
...
- [x] 定域：迁出 background run façade wrapper，包括后台 plan/implementation/QA/merge-test 启动与通知 wrapper、completion wait/failed/pending-action/notification wrapper、sender scheduling/reply wrapper 和 execution policy artifact wrapper；不迁后台 executor/notifier/orchestration 核心实现、`start_run()` 主体、active run reconcile 主体、runner 调度或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 background run façade wrapper。
...
- [x] 定域：迁出 manifest/artifact/session façade wrapper，包括 existing/fresh artifact set、manifest 构造、Codex attach/resume 命令、session id 读取、runner manifest name 和 manifest session metadata 写回；不迁 `start_run()` 主体、active run reconcile 主体、runner 调度、workspace/git 或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 manifest/artifact/session façade wrapper。
...
### 阶段 248：Task 37 Orchestrator 500 行治理第二十一切片
- [x] 定域：迁出 merge-test readiness façade wrapper，包括 merge-test blocker、blocked task 风险放行 host 写回、blocked assessment fact collection、latest implementation run/source branch/disallowed reason wrapper 和 QA evidence wrapper；不迁 `start_run()` 主体、workspace/git/checkpoint 实现、merge-test readiness 纯评估规则、runner 调度或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 merge-test readiness façade wrapper。
- [x] RED：focused architecture test 先失败于 `orchestrator.py` 仍定义 merge-test readiness façade wrapper。
- [x] 实现：新增 merge-test façade mixin 承接 wrapper；`CodingOrchestrator` 继承 mixin 并删除主文件内对应实现。
- [x] 验证：运行 focused architecture test、merge-test 相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 249：Task 37 Orchestrator 500 行治理第二十二切片
- [x] 定域：迁出 runtime/failure façade wrapper，包括 flag helper、project/workflow wrapper、plan report session/execution policy/timeout wrapper、runner failed result 和 checkpoint failed result；不迁 `start_run()` 主体、runner dispatch、artifact service 纯写入、workspace/git/checkpoint 实现或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 runtime/failure façade wrapper。
- [x] RED：focused architecture test 先失败于 `orchestrator.py` 仍定义 11 个 runtime/failure façade wrapper。
- [x] 实现：新增 runtime façade mixin 承接 wrapper；`CodingOrchestrator` 继承 mixin 并删除主文件内对应实现。
- [x] 验证：运行 focused architecture test、runtime/run 相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 250：Task 37 Orchestrator 500 行治理第二十三切片
- [x] 定域：迁出 active run reconcile façade wrapper，包括 completed active run reconcile host shell 和 agent run lookup helper；不迁 `start_run()` 主体、fresh completion writeback、runner dispatch、workspace/git/checkpoint 实现或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 active run reconcile façade wrapper。
- [x] RED：focused architecture test 先失败于 `orchestrator.py` 仍定义 2 个 active run reconcile façade wrapper。
- [x] 实现：新增 active run façade mixin 承接 wrapper；`CodingOrchestrator` 继承 mixin 并删除主文件内对应实现，保留主模块级 `run_reconcile_writeback_service` import 兼容既有 monkeypatch。
- [x] 验证：运行 focused architecture test、active reconcile 相邻回归、py_compile、文档/架构测试、YAML 解析、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete

### 阶段 251：Task 37 Orchestrator 500 行治理第二十四切片
- [x] 定域：迁出 bootstrap/runtime binding façade wrapper，包括 `__post_init__`、`set_dispatch_tool`、`from_default_config` 和默认运行根 helper；不迁 `handle_gateway_event()`、`pre_llm_call()`、`start_run()` 主体、runner dispatch、workspace/git/checkpoint 实现或 run lifecycle。
- [x] TDD：扩展架构测试，要求 `orchestrator.py` 不再直接定义 bootstrap façade wrapper。
- [x] RED：focused architecture test 先失败于 `orchestrator.py` 仍定义 4 个 bootstrap façade wrapper。
- [x] 实现：新增 bootstrap façade mixin 承接初始化、默认配置和 Hermes runtime binding；`CodingOrchestrator` 继承 mixin 并删除主文件内对应实现。
- [x] 验证：运行 focused architecture test、bootstrap/Gateway/CLI 相邻回归、py_compile、architecture guard、diff check 和 release readiness no-smoke。
- **状态：** complete
...
- [x] 定域：只让 task creation source helper 消费 `SourceProjection`；不改 source reader、ledger schema、source_context 持久化、Gateway 或 run lifecycle。
- [x] TDD：扩展 `tests/test_task_service.py`，patch `source_projection_from_context()` 并确认初始状态、needs-human gate、message/requirement summary 来自 projection。
- [x] RED：focused tests 先失败于旧 helper 仍返回 legacy `read_status`/gate/title/summary 结果。
- [x] 实现：`task_utils.source_status_from_context()`、`source_context_requires_human()`、`message_summary()` 和 `requirement_summary()` 改为消费 `SourceProjection`。
- [x] 文档：将 Task 31 标为 Complete，并同步技术方案、计划、进度、发现、项目地图、组件合同、约定和 machine-readable context。
- [x] 验证：运行 task creation helper focused、TaskService/source projection/source flow 相邻回归、py_compile、文档完成态 focused、architecture guard、diff check 和完整单测。
- **状态：** complete

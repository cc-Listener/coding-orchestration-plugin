...
- **状态：** complete
- 背景：
  - `models.py` 维护任务状态、任务类型、phase、run mode、runner capability、run manifest、artifact set 和状态显示 helper，属于公共模型合同边界，不应继续散落在 `coding_orchestration/` 包根。
  - 本切片只做同名包化，并通过 `coding_orchestration/models/__init__.py` 保留 `from coding_orchestration.models import ...` 稳定导出；状态机、RunService、ports、runner/workspace/git 和 run lifecycle 不属于本切片。
- 执行的操作：
  - 扩展 `tests/test_architecture_module_layout.py`，要求包根不再保留 `models.py`，并要求 `coding_orchestration/models/__init__.py` 与 `coding_orchestration/models/contracts.py` 存在。
  - RED 已确认：focused layout test 先失败，失败点为包根旧文件 `models.py` 仍存在。
  - 将模型合同移入 `coding_orchestration/models/contracts.py`，新增 `coding_orchestration/models/__init__.py` 保留稳定导出。
  - 更新 architecture guard 边界扫描路径，并同步技术方案、项目地图、组件合同、约定、machine-readable context 和发现中的 canonical path。
- 当前边界：
  - `coding_orchestration/models/contracts.py`：只做公共枚举、runner capability、manifest、artifact contract 和状态显示 helper。
  - `coding_orchestration/models/__init__.py`：只做稳定导出，不承载业务逻辑。
  - 状态机、RunService、ports、runner/workspace/git 和 run lifecycle 均未迁移。
- 已验证：
  - RED：`rtk python3 -m unittest tests.test_architecture_module_layout -v` 先失败，失败点为包根旧文件 `models.py`。
  - Focused GREEN：`rtk python3 -m unittest tests.test_architecture_module_layout tests.test_state_machine tests.test_run_service tests.test_run_status_transition_service tests.test_run_artifact_paths tests.test_architecture_guard -v`：83 tests passed。
  - 编译检查：`rtk python3 -m compileall coding_orchestration tests`：passed。
  - YAML 解析：`rtk python3 -c ...`：passed，输出 `yaml ok`。
  - 架构检查：`rtk proxy python3 scripts/architecture_guard.py`：passed，no findings。
  - 格式检查：`rtk git diff --check`：passed。
  - Release gate no-smoke：`rtk proxy python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 1002 tests passed，architecture guard no findings，敏感扫描 no findings。
- 当前包根：
  - `coding_orchestration/` 包根 `.py` 数量：3（不含 `__init__.py`）。
  - 包根 `.py` 总行数：1112。
  - 剩余包根文件：`ledger.py`、`orchestrator.py`、`run_orchestration_service.py`。
- 剩余风险：
  - 剩余包根文件集中在 ledger façade、orchestrator façade 和 run orchestration owner；继续治理时应优先采用 façade 包化或 projection/service 分拆，不迁 runner/workspace/git/run lifecycle 核心实现。
...
- **状态：** complete
- 背景：
  - `codex_reuse.py` 只维护 Hermes terminal Codex CLI、Hermes openai-codex provider 和 direct Codex CLI fallback 的 backend decision。
  - 该职责属于 runner helper 边界，不应继续散落在 `coding_orchestration/` 包根；`runner_router.py` 继续作为 runner 选择入口暂留包根。
- 执行的操作：
  - 扩展 `tests/test_architecture_module_layout.py`，要求 Codex reuse helper 位于 `coding_orchestration/runners/`。
  - RED 已确认：focused layout test 先失败，失败点为 `codex_reuse.py` 仍在包根。
  - 将 `codex_reuse.py` 移入 `coding_orchestration/runners/codex_reuse.py`。
  - 更新 `runner_router.py`、`runners/hermes_autonomous_codex.py`、`tests/test_codex_reuse.py` 和当前事实文档中的 import / canonical 路径。
- 当前边界：
  - `coding_orchestration/runners/codex_reuse.py`：只维护 `CodexReuseStrategy` / `CodexBackendDecision`，不读取 runner router 配置、不实现 Hermes runtime adapter、不检测 auth、不承载 runner/workspace/git 或 run lifecycle。
  - `coding_orchestration/runner_router.py`：继续作为 runner 选择入口，暂不迁移。
- 已验证：
  - RED：`rtk python3 -m unittest tests.test_architecture_module_layout -v` 先失败，失败点为 `codex_reuse.py` 仍在包根。
  - Focused GREEN：`rtk python3 -m unittest tests.test_architecture_module_layout -v`：1 test passed。
  - Codex reuse / runner 相邻回归：`rtk python3 -m unittest tests.test_codex_reuse tests.test_hermes_runtime_runner tests.test_coding_diagnostics_command_executor tests.test_orchestrator_run_flow tests.test_architecture_module_layout -v`：21 tests passed。
  - 编译检查：`rtk python3 -m py_compile coding_orchestration/runners/codex_reuse.py coding_orchestration/runner_router.py coding_orchestration/runners/hermes_autonomous_codex.py tests/test_codex_reuse.py tests/test_architecture_module_layout.py`：passed。
  - YAML 解析：`rtk python3 -c ...`：passed，输出 `yaml ok`。
  - 架构检查：`rtk python3 scripts/architecture_guard.py`：passed，no findings。
  - 格式检查：`rtk git diff --check`：passed。
  - Release gate no-smoke：`rtk python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 996 tests passed，architecture guard no findings，敏感扫描 no findings。
- 当前包根：
  - `coding_orchestration/` 包根 `.py` 数量：12。
  - 包根 `.py` 总行数：2389。
  - 剩余包根文件：`cli.py`、`config.py`、`ledger.py`、`models.py`、`orchestrator.py`、`plugin_tools.py`、`ports.py`、`run_orchestration_service.py`、`runner_router.py`、`state_machine.py`、`status_policy.py`、`workspace_checkpoint_service.py`。
- 剩余风险：
  - 剩余包根多为公共入口、核心模型/状态合同、runner/workspace/run orchestration 或 ledger/storage façade；后续应继续按低风险单域切片处理，不一次性迁 runner/workspace/git/run lifecycle。
...
- **状态：** complete
- 背景：
  - `command_catalog.py` 是 `/coding` command facts、help/listing 文案、rewrite context 和 allowed commands 的单一事实源。
  - `command_rewriter.py` 维护 Coding Mode LLM rewrite prompt、JSON 解析和 fallback shape。
  - 两者属于 command rule helper 边界，不应继续散落在 `coding_orchestration/` 包根；收拢到 `coding_orchestration/commands/` 后，Gateway/command host shell 只消费 command package。
- 执行的操作：
  - 扩展 `tests/test_architecture_module_layout.py`，要求 `command_*.py` helper 位于 `coding_orchestration/commands/`。
  - RED 已确认：focused test 先失败，失败点为 `command_catalog.py` 和 `command_rewriter.py` 仍在包根。
  - 将两个模块移入 `coding_orchestration/commands/`，更新 Gateway coding mode executor、help executor、bootstrap façade 和测试 import。
  - 同步项目地图、组件合同、约定、machine-readable context、技术方案、发现和目录治理计划。
- 当前边界：
  - `coding_orchestration/commands/command_catalog.py`：只维护命令事实源、help/listing 文案和 rewrite context 数据，不承载 Gateway route/controller、host shell、副作用或 run lifecycle。
  - `coding_orchestration/commands/command_rewriter.py`：只维护 LLM rewrite prompt、响应解析和 fallback shape，不执行命令、不创建 task、不启动 runner、不发送 Gateway 回复。
- 已验证：
  - RED：`rtk python3 -m unittest tests.test_architecture_module_layout -v` 先失败，失败点为两个 command helper 仍在包根。
  - Focused GREEN：`rtk python3 -m unittest tests.test_architecture_module_layout -v`：1 test passed。
  - Command catalog/rewrite/Gateway 相邻回归：`rtk python3 -m unittest tests.test_command_catalog tests.test_coding_help_command_executor tests.test_gateway_rewrite_flow tests.test_gateway_command_group_flow tests.test_gateway_project_task_flow tests.test_gateway_feedback_flow tests.test_gateway_change_continue_flow tests.test_gateway_task_control_flow tests.test_gateway_safety_lifecycle_flow tests.test_status_reconcile_flow -v`：69 tests passed。
  - 编译检查：`rtk python3 -m py_compile coding_orchestration/commands/command_catalog.py coding_orchestration/commands/command_rewriter.py coding_orchestration/gateway/gateway_coding_mode_executor.py coding_orchestration/coding_commands/coding_help_command_executor.py coding_orchestration/orchestrator_facades/orchestrator_bootstrap_facade.py tests/test_command_catalog.py tests/test_architecture_module_layout.py`：passed。
  - YAML 解析：`rtk python3 -c 'import yaml; yaml.safe_load(open("contracts/project-context.yaml", encoding="utf-8")); print("yaml ok")'`：passed。
  - 架构检查：`rtk python3 scripts/architecture_guard.py`：passed，no findings。
  - 格式检查：`rtk git diff --check`：passed。
  - Release gate no-smoke：`rtk python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 996 tests passed，architecture guard no findings，敏感扫描 no findings。
- 剩余风险：
  - 包根仍保留 prompt/context/report、runner/workspace/run orchestration 等跨域或核心模块；后续应继续优先处理职责单一、引用面可控的模块族，避免一次性迁 runner/workspace/git 核心。
...
- **状态：** complete
- 背景：
  - `run_completion_writeback_service.py` 维护 fresh completed run 的 completion projection、stale observation、状态 transition、ledger/session/summary/project writeback 和 final result payload 协调。
  - 该模块已经是注入 callback 的 run host service，适合收拢到 `coding_orchestration/run/services/`；不应继续散落在包根，也不应吸收 active reconcile、manifest、run orchestration projection 或 runner/workspace/git 生命周期。
- 执行的操作：
  - 扩展 `tests/test_architecture_module_layout.py`，要求 `run_completion_writeback_service.py` 位于 `coding_orchestration/run/services/`。
  - RED 已确认：focused test 先失败，失败点为包根仍存在 `run_completion_writeback_service.py`。
  - 将 `coding_orchestration/run_completion_writeback_service.py` 移动到 `coding_orchestration/run/services/run_completion_writeback_service.py`，并修正相对 import。
  - 更新 `orchestrator.py` 从 `run.services` 导入该模块，保留 `orchestrator_module.run_completion_writeback_service` 模块变量以兼容既有 monkeypatch 合同。
  - 更新 run completion writeback 测试 import，并同步项目地图、组件合同、约定、machine-readable context、技术方案、发现和目录治理计划。
- 当前边界：
  - `coding_orchestration/run/services/run_completion_writeback_service.py`：只协调 fresh completed run 的 report artifact、状态 transition、ledger/session/summary/project writeback 和 result payload。
  - 本切片未迁 active reconcile、run manifest、run orchestration projection、runner/workspace/git 或 run lifecycle。
- 已验证：
  - RED：`rtk python3 -m unittest tests.test_architecture_module_layout -v` 先失败，失败点为 `run_completion_writeback_service.py` 仍在包根。
  - Focused GREEN：`rtk python3 -m unittest tests.test_architecture_module_layout -v`：1 test passed。
  - Run completion writeback contract：`rtk python3 -m unittest tests.test_run_completion_writeback_service -v`：3 tests passed。
  - Completion/reconcile/run flow 相邻回归：`rtk python3 -m unittest tests.test_run_completion_writeback_service tests.test_run_reconcile_writeback_service tests.test_run_status_transition_service tests.test_orchestrator_run_flow tests.test_run_session_writeback_service tests.test_run_project_writeback_service tests.test_run_orchestration_start_rules -v`：51 tests passed。
  - 文档合同回归：`rtk python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_module_layout -v`：17 tests passed。
  - 编译检查：`rtk python3 -m py_compile coding_orchestration/run/services/run_completion_writeback_service.py coding_orchestration/orchestrator.py tests/test_run_completion_writeback_service.py tests/test_architecture_module_layout.py`：passed。
  - YAML 解析：`rtk python3 -c "import yaml; yaml.safe_load(open('contracts/project-context.yaml', encoding='utf-8')); print('yaml ok')"`：passed。
  - 架构检查：`rtk python3 scripts/architecture_guard.py`：passed，no findings。
  - 格式检查：`rtk git diff --check`：passed。
  - Release gate no-smoke：`rtk python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 996 tests passed，architecture guard no findings，敏感扫描 no findings。
- 剩余风险：
  - Run 域包根仍保留 `run_reconcile_writeback_service.py`、`run_manifest_service.py` 和 `run_orchestration_service.py`；其中 active reconcile writeback 是下一片最小风险候选，manifest/orchestration helper 需更谨慎拆分。

### 阶段 275：Source resolver 子包治理
- **状态：** complete
- 背景：
  - `source_resolver.py` 维护 `SourceResolver.resolve_source_result()` / legacy `resolve_source()`、Lark preflight 和 Meegle preflight，是 source 域 resolver host 边界。
  - Source helper、projection、Meegle adapter 和 context repair 已先行收拢；resolver 不应继续散落在 `coding_orchestration/` 包根，也不应吸收 reader、ledger 或 run lifecycle 职责。
- 执行的操作：
  - 扩展 `tests/test_architecture_module_layout.py`，要求 `source_resolver.py` 位于 `coding_orchestration/source/`。
  - RED 已确认：focused test 先失败，失败点为包根仍存在 `source_resolver.py`。
  - 将 `coding_orchestration/source_resolver.py` 移动到 `coding_orchestration/source/source_resolver.py`，并修正 Feishu reader、Meegle adapter 和 `SourceResult` 的相对 import。
  - 更新 orchestrator bootstrap façade、install integration、tool façade 错误提示和 SourceResolver/Meegle 测试 import 到新包路径。
  - 同步项目地图、组件合同、约定、machine-readable context、技术方案、发现和目录治理计划。
- 当前边界：
  - `coding_orchestration/source/source_resolver.py`：统一输出 `SourceResult`，保留旧 `resolve_source()` 兼容 dict context，并维护 Lark / Meegle preflight。
  - 本切片未迁 Feishu reader、Meegle adapter、source context repair、ledger schema、runner/workspace/git 或 run lifecycle。
- 已验证：
  - RED：`rtk python3 -m unittest tests.test_architecture_module_layout -v` 先失败，失败点为 `source_resolver.py` 仍在包根。
  - Focused GREEN：`rtk python3 -m unittest tests.test_architecture_module_layout -v`：1 test passed。
  - SourceResolver/Feishu/Meegle/diagnostics/CLI 相邻回归：`rtk python3 -m unittest tests.test_source_resolver tests.test_meegle_reader tests.test_source_flow tests.test_source_plan_flow tests.test_orchestrator_tools tests.test_coding_diagnostics_command_executor tests.test_coding_cli tests.test_install tests.test_docs_and_install_entry -v`：95 tests passed。
  - 文档合同回归：`rtk python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_module_layout -v`：17 tests passed。
  - 编译检查：`rtk python3 -m py_compile ...`：passed。
  - YAML 解析：`rtk python3 -c "import yaml; yaml.safe_load(open('contracts/project-context.yaml', encoding='utf-8')); print('yaml ok')"`：passed。
  - 架构检查：`rtk python3 scripts/architecture_guard.py`：passed，no findings。
  ...
  - Release gate no-smoke：`rtk python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 996 tests passed，architecture guard no findings，敏感扫描 no findings。
- 剩余风险：
  - Source 域包根治理已基本收口；后续集中转向 run/project/其他包根模块，仍需逐片判断公共入口是否应保留在包根。
...
- **状态：** complete
- 背景：
  - `source_projection.py` 维护 `SourceResult` / legacy `source_context` 到 `SourceProjection` 的稳定投影，已被 prompt/context/status/manifest/run-preflight/dashboard source health/source-resolve diagnostic payload/task creation helper 多处消费。
  - 该模块属于通用 source 投影边界，不应继续散落在 `coding_orchestration/` 包根，也不应并入 Feishu reader adapter 或 resolver。
- 执行的操作：
  - 扩展 `tests/test_architecture_module_layout.py`，要求 `source_projection.py` 位于 `coding_orchestration/source/`。
  - RED 已确认：focused test 先失败，失败点为包根仍存在 `source_projection.py`。
  - 将 `coding_orchestration/source_projection.py` 移动到 `coding_orchestration/source/source_projection.py`，并修正相对 import。
  - 更新 prompt、context assembler、run manifest、run context artifact、TaskService helper、source context repair、orchestrator façade 和测试 import 到新包路径。
  - 保留 `orchestrator.py` 中 `from .source import source_projection` 兼容模块变量，继续支持现有 monkeypatch 合同。
  - 同步项目地图、组件合同、约定、machine-readable context、技术方案、发现和目录治理计划。
- 当前边界：
  - `coding_orchestration/source/source_projection.py`：只承接来源稳定投影和 context-index 序列化 payload。
  - 本切片未迁 `source_resolver.py`、`source_context_repair_service.py`、`meegle_reader.py`、reader transport、ledger schema、runner/workspace/git 或 run lifecycle。
- 已验证：
  - RED：`rtk python3 -m unittest tests.test_architecture_module_layout -v` 先失败，失败点为 `source_projection.py` 仍在包根。
  - Focused GREEN：`rtk python3 -m unittest tests.test_architecture_module_layout -v`：1 test passed。
  - Source projection 相邻回归：`rtk python3 -m unittest tests.test_source_projection tests.test_prompt_templates tests.test_run_context_artifact_service tests.test_context_assembler tests.test_task_service tests.test_run_manifest_service tests.test_source_plan_flow tests.test_dashboard_api_contract tests.test_source_flow tests.test_orchestrator_tools tests.test_coding_cli -v`：95 tests passed。
  - 文档合同回归：`rtk python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_module_layout -v`：17 tests passed。
  - 编译检查：`rtk python3 -m py_compile ...`：passed。
  - YAML 解析：`rtk python3 -c "import yaml; yaml.safe_load(open('contracts/project-context.yaml', encoding='utf-8')); print('yaml ok')"`：passed。
  - 架构检查：`rtk python3 scripts/architecture_guard.py`：passed，no findings。
  ...
  - Release gate no-smoke：`rtk python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 996 tests passed，architecture guard no findings，敏感扫描 no findings。
- 剩余风险：
  - 包根仍保留 `source_resolver.py`、`source_context_repair_service.py` 和 `meegle_reader.py`；这三者引用面不同，需要后续独立切片继续评估。
...
- **状态：** complete
- 背景：
  - 阶段 248 已把 merge-test readiness façade 迁入 `orchestrator_merge_test_facade.py`；`orchestrator.py` 仍直接承载末尾 runtime/failure façade wrapper，包括 flag helper、project/workflow wrapper、plan report session/execution policy/timeout wrapper、runner failed result 和 checkpoint failed result。
  - 本阶段选择 runtime/failure façade 作为低风险 host wrapper 切片；不迁 `start_run()` 主体、runner dispatch、artifact service 纯写入、workspace/git/checkpoint 实现或 run lifecycle。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 runtime/failure façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 11 个 runtime/failure façade wrapper。
  - 新增 `coding_orchestration/orchestrator_runtime_facade.py`，用 `OrchestratorRuntimeFacadeMixin` 承接 runtime/failure façade wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorRuntimeFacadeMixin`，删除主文件内对应 runtime/failure façade 实现和迁移后的无用 import。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：661 行。
  - `coding_orchestration/orchestrator_runtime_facade.py`：119 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_runtime_facade_methods -v` 先失败 11 个 subTest，失败点为 runtime/failure façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_runtime_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_run_service tests.test_run_dispatch_service tests.test_run_orchestration_start_rules tests.test_run_orchestration_plan_report_session tests.test_run_session_projection tests.test_command_run_flow tests.test_status_reconcile_flow tests.test_task_service -v`：76 tests passed。
  - 文档/架构测试：`rtk proxy python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_guard -v`：41 tests passed。
  - YAML 解析：`rtk proxy python3 -c 'import yaml; yaml.safe_load(open("contracts/project-context.yaml", encoding="utf-8")); print("yaml ok")'`：passed。
  - 架构检查：`rtk proxy python3 scripts/architecture_guard.py`：passed，仅 watch `coding_orchestration/orchestrator.py: 661 lines`。
  - Release gate no-smoke：`rtk proxy python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 992 tests passed，敏感扫描 no findings。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 `handle_gateway_event()`、active run reconcile、`start_run()` 主体、runner dispatch、workspace/git/checkpoint、artifact service 纯写入或 run lifecycle。

### 阶段 248：Task 37 Orchestrator 500 行治理第二十一切片
- **状态：** complete
- 背景：
  - 阶段 247 已把 prompt/context façade 迁入 `orchestrator_prompt_context_facade.py`；`orchestrator.py` 仍直接承载 merge-test readiness façade wrapper，包括 merge-test blocker、blocked task 风险放行 host 写回、blocked assessment fact collection、latest implementation run/source branch/disallowed reason wrapper 和 QA evidence wrapper。
  - 本阶段选择 merge-test readiness façade 作为低风险 host wrapper 切片；不迁 `start_run()` 主体、workspace/git/checkpoint 实现、merge-test readiness 纯评估规则、runner 调度或 run lifecycle。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 merge-test readiness façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 7 个 merge-test readiness façade wrapper。
  - 新增 `coding_orchestration/orchestrator_merge_test_facade.py`，用 `OrchestratorMergeTestFacadeMixin` 承接 merge-test readiness façade wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorMergeTestFacadeMixin`，删除主文件内对应 merge-test readiness façade 实现和迁移后的无用 import。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：768 行。
  - `coding_orchestration/orchestrator_merge_test_facade.py`：150 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_merge_test_facade_methods -v` 先失败 7 个 subTest，失败点为 merge-test readiness façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_merge_test_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_merge_test_readiness_service tests.test_merge_test_readiness_flow tests.test_merge_test_blocked_flow tests.test_merge_test_qa_gate_flow tests.test_merge_test_basic_flow tests.test_coding_merge_test_command_executor tests.test_gateway_command_executor -v`：41 tests passed。
  - 文档/架构测试：`rtk proxy python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_guard -v`：40 tests passed。
  - YAML 解析：`rtk proxy python3 -c 'import yaml; yaml.safe_load(open("contracts/project-context.yaml", encoding="utf-8")); print("yaml ok")'`：passed。
  - 架构检查：`rtk proxy python3 scripts/architecture_guard.py`：passed，仅 watch `coding_orchestration/orchestrator.py: 768 lines`。
  - 格式检查：`rtk proxy git diff --check`：passed。
  - Release gate no-smoke：`rtk proxy python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 991 tests passed，敏感扫描 no findings。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 `handle_gateway_event()`、active run reconcile、`start_run()`、runtime helper、failure result wrapper、runner 调度、workspace/git 或 run lifecycle。

### 阶段 247：Task 37 Orchestrator 500 行治理第二十切片
- **状态：** complete
- 背景：
  - 阶段 246 已把 manifest/artifact/session façade 迁入 `orchestrator_manifest_facade.py`；`orchestrator.py` 仍直接承载 prompt/context façade wrapper，包括 resumed session 增量上下文、wiki docs/refs、context artifacts、dependency/sibling task context、confirmed plan / merge-test context、report summary markdown 和 wiki ref 投影 wrapper。
  - 本阶段选择 prompt/context façade 作为低风险 host wrapper 切片；不迁 `start_run()` 主体、context artifact service 纯写入、prompt projection、manifest 构造、workspace/git、runner 调度或 run lifecycle。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 prompt/context façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 10 个 prompt/context façade wrapper。
  - 新增 `coding_orchestration/orchestrator_prompt_context_facade.py`，用 `OrchestratorPromptContextFacadeMixin` 承接 prompt/context façade wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorPromptContextFacadeMixin`，删除主文件内对应 prompt/context façade 实现；保留 `orchestrator.py` 的 `run_context_artifact_service` 模块级 import，兼容既有 execution policy wrapper monkeypatch。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：914 行。
  - `coding_orchestration/orchestrator_prompt_context_facade.py`：191 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_prompt_context_facade_methods -v` 先失败 10 个 subTest，失败点为 prompt/context façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_prompt_context_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_run_report_artifact_service tests.test_run_context_artifact_service tests.test_context_assembler tests.test_plan_run_flow tests.test_implementation_session_flow tests.test_ledger_wiki_orchestrator -v`：38 tests passed。
  - 文档/架构测试：`rtk proxy python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_guard -v`：39 tests passed。
  - YAML 解析：`rtk proxy python3 -c 'import yaml; yaml.safe_load(open("contracts/project-context.yaml", encoding="utf-8")); print("yaml ok")'`：passed。
  - 架构检查：`rtk proxy python3 scripts/architecture_guard.py`：passed，仅 watch `coding_orchestration/orchestrator.py: 914 lines`。
  - 格式检查：`rtk proxy git diff --check`：passed。
  - Release gate no-smoke：`rtk proxy python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 990 tests passed，敏感扫描 no findings。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 `handle_gateway_event()`、active run reconcile、`start_run()`、merge-test readiness façade、runtime helper、runner 调度、workspace/git 或 run lifecycle。
...
- **状态：** complete
- 背景：
  - 阶段 244 已把 background run façade 迁入 `orchestrator_background_facade.py`；`orchestrator.py` 仍直接承载 status/report policy façade wrapper，包括 report status details、implementation status normalize、verification limitations 补齐和 run result 到 task status/phase 映射 wrapper。
  - 本阶段选择 status/report policy façade 作为低风险 host wrapper 切片；不迁 `status_policy.py` 纯规则、状态机、`start_run()` 主体、active run reconcile 主体、runner 调度或 run lifecycle。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 status/report policy façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 11 个 status/report policy façade wrapper。
  - 新增 `coding_orchestration/orchestrator_status_policy_facade.py`，用 `OrchestratorStatusPolicyFacadeMixin` 承接 status/report policy façade wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorStatusPolicyFacadeMixin`，删除主文件内对应 status/report policy façade 实现和迁移后的无用 import。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：1221 行。
  - `coding_orchestration/orchestrator_status_policy_facade.py`：77 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_status_policy_facade_methods -v` 先失败 11 个 subTest，失败点为 status/report policy façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_status_policy_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_status_policy tests.test_run_service tests.test_status_reconcile_flow tests.test_run_orchestration_start_rules tests.test_task_lifecycle_guard_service -v`：56 tests passed。
  - 文档/架构测试：`rtk proxy python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_guard -v`：37 tests passed。
  - YAML 解析：`rtk proxy python3 -c 'import yaml; yaml.safe_load(open("contracts/project-context.yaml", encoding="utf-8")); print("yaml ok")'`：passed。
  - 架构检查：`rtk proxy python3 scripts/architecture_guard.py`：passed，仅 watch `coding_orchestration/orchestrator.py: 1221 lines`。
  - 格式检查：`rtk proxy git diff --check`：passed。
  - Release gate no-smoke：`rtk proxy python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 988 tests passed，敏感扫描 no findings。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 `status_policy.py` 纯规则、状态机、`handle_gateway_event()`、active run reconcile、runner 调度、`start_run()`、run lifecycle 或其他剩余 façade。
...
- **状态：** complete
- 背景：
  - 阶段 242 已把 task runtime façade 迁入 `orchestrator_task_runtime_facade.py`；`orchestrator.py` 仍直接承载 workspace checkpoint façade wrapper，包括 implementation/merge-test workspace 查找、QA artifact 收集、QA/merge-test checkpoint 准备、dirty/head/clean checkpoint 观测、source branch/base branch 投影、task short id/slug 和 latest existing implementation workspace wrapper。
  - 本阶段选择 workspace checkpoint façade 作为低风险 host wrapper 切片；不迁 workspace/git/checkpoint 核心实现、`start_run()` 主体、active run reconcile 主体、runner 调度或 run lifecycle。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 workspace checkpoint façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 13 个 workspace checkpoint façade wrapper。
  - 新增 `coding_orchestration/orchestrator_workspace_facade.py`，用 `OrchestratorWorkspaceFacadeMixin` 承接 workspace checkpoint façade wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorWorkspaceFacadeMixin`，删除主文件内对应 workspace façade 实现和迁移后的无用 import。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：1370 行。
  - `coding_orchestration/orchestrator_workspace_facade.py`：58 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_workspace_facade_methods -v` 先失败 13 个 subTest，失败点为 workspace checkpoint façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_workspace_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_workspace_checkpoint_service tests.test_implementation_workspace_flow tests.test_implementation_session_flow tests.test_qa_flow tests.test_merge_test_basic_flow tests.test_merge_test_qa_gate_flow tests.test_run_checkpoint_preparation_service tests.test_run_evidence_observation_service tests.test_command_run_flow tests.test_status_reconcile_flow -v`：69 tests passed。
  - 文档/架构测试、YAML 解析、architecture guard、格式检查和 Release gate no-smoke 已在提交前验证通过。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 workspace/git/checkpoint 核心实现、`handle_gateway_event()`、active run reconcile、runner 调度、`start_run()`、run lifecycle 或状态机核心。

### 阶段 242：Task 37 Orchestrator 500 行治理第十五切片
- **状态：** complete
- 背景：
  - 阶段 241 已把 task/source façade 迁入 `orchestrator_task_source_facade.py`；`orchestrator.py` 仍直接承载 task runtime façade wrapper，包括 Kanban/status host wrapper、task list/status event wrapper、feedback active-task wrapper、active task/coding mode binding wrapper、task lifecycle wrapper、人工决策记录和 RunService gate wrapper。
  - 本阶段选择 task runtime façade 作为低风险 host wrapper 切片；不迁 `start_run()` 主体、active run reconcile 主体、runner/workspace/git 或 run lifecycle 核心。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 task runtime façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 34 个 task runtime façade wrapper。
  - 新增 `coding_orchestration/orchestrator_task_runtime_facade.py`，用 `OrchestratorTaskRuntimeFacadeMixin` 承接 task runtime façade wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorTaskRuntimeFacadeMixin`，删除主文件内对应 task runtime façade 实现和迁移后的无用 import。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：1419 行。
  - `coding_orchestration/orchestrator_task_runtime_facade.py`：204 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_task_runtime_facade_methods -v` 先失败 34 个 subTest，失败点为 task runtime façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_task_runtime_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_kanban_sync_service tests.test_task_lifecycle_guard_service tests.test_coding_feedback_command_executor tests.test_coding_status_command_executor tests.test_coding_task_list_command_executor tests.test_gateway_coding_mode_executor tests.test_gateway_command_executor tests.test_cancel_restore_flow tests.test_command_run_flow tests.test_status_reconcile_flow -v`：65 tests passed。
  - 文档/架构测试、YAML 解析、architecture guard、格式检查和 Release gate no-smoke 已在提交前验证通过。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 `handle_gateway_event()`、active run reconcile、runner/workspace/git、`start_run()`、run lifecycle 或状态机核心。

### 阶段 241：Task 37 Orchestrator 500 行治理第十四切片
- **状态：** complete
- 背景：
  - 阶段 240 已把 project/profile/active-project façade 迁入 `orchestrator_project_facade.py`；`orchestrator.py` 仍直接承载 task/source façade wrapper，包括 task create command wrapper、TaskService source/status payload wrapper、source context 读取/修复/enrich wrapper、event/media ledger helper 和 draft knowledge source refs wrapper。
  - 本阶段选择 task/source façade 作为边界清楚的 host wrapper 切片；不迁 active task/coding mode binding、feedback command host shell、run lifecycle、runner/workspace/git、`start_run()` 或状态机核心。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 task/source façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 33 个 task/source façade wrapper。
  - 新增 `coding_orchestration/orchestrator_task_source_facade.py`，用 `OrchestratorTaskSourceFacadeMixin` 承接 task create command wrapper、TaskService source/status payload wrapper、source context 读取/修复/enrich wrapper、event/media ledger helper 和 draft knowledge source refs wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorTaskSourceFacadeMixin`，删除主文件内对应 task/source façade 实现和迁移后的无用 import。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：1608 行。
  - `coding_orchestration/orchestrator_task_source_facade.py`：240 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_task_source_facade_methods -v` 先失败 33 个 subTest，失败点为 task/source façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_task_source_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_task_service tests.test_task_status_payload tests.test_source_context_repair_service tests.test_source_flow tests.test_source_plan_flow tests.test_gateway_command_executor tests.test_gateway_coding_mode_executor tests.test_coding_feedback_command_executor tests.test_plan_run_flow -v`：59 tests passed。
  - 文档/架构测试、YAML 解析、architecture guard、格式检查和 Release gate no-smoke 已在提交前验证通过。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 active task/coding mode binding、feedback command host shell、active run reconcile、runner/workspace/git、`start_run()`、run lifecycle 或状态机核心。
...
- **状态：** complete
- 背景：
  - 阶段 238 已把 diagnostics/dashboard façade 迁入 `orchestrator_diagnostics_facade.py`；`orchestrator.py` 仍直接承载 Gateway route façade wrapper，包括 immediate/custom route dispatch、Coding Mode rewrite wrapper、pending action/rewrite binding wrapper、确认/取消词 wrapper、commands listing route、dedupe/授权/looks-like-task wrapper。
  - 本阶段选择 Gateway route façade 作为高收益 host wrapper 切片；不迁 `handle_gateway_event()` 入口、task creation/source helper、active task/project binding 业务、runner/workspace/git、`start_run()` 或 run lifecycle。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再直接定义 Gateway façade wrapper。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 35 个 Gateway façade wrapper。
  - 新增 `coding_orchestration/orchestrator_gateway_facade.py`，用 `OrchestratorGatewayFacadeMixin` 承接 Gateway route façade、Coding Mode rewrite wrapper、pending action/rewrite binding wrapper、commands listing route、dedupe/授权/looks-like-task wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorGatewayFacadeMixin`，删除主文件内对应 Gateway façade 实现和迁移后的 Gateway command regex alias。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：2016 行。
  - `coding_orchestration/orchestrator_gateway_facade.py`：237 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_gateway_facade_methods -v` 先失败 35 个 subTest，失败点为 Gateway façade wrapper 仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_gateway_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_gateway_command_controller tests.test_gateway_command_group_flow tests.test_gateway_coding_mode_executor tests.test_gateway_coding_mode_lifecycle_flow tests.test_gateway_rewrite_flow tests.test_gateway_pending_action_executor tests.test_gateway_pending_confirmation_flow tests.test_gateway_task_control_flow tests.test_gateway_project_task_flow tests.test_gateway_natural_language_command_flow tests.test_gateway_change_continue_flow tests.test_gateway_feedback_flow tests.test_gateway_safety_lifecycle_flow tests.test_gateway_trigger -v`：passed。
  - 文档/架构测试、YAML 解析、architecture guard、格式检查和 Release gate no-smoke 已在提交前验证通过。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 `handle_gateway_event()` 入口、task creation/source helper、active run reconcile、active task/project binding 业务、runner/workspace/git、`start_run()`、run lifecycle 或状态机核心。

### 阶段 238：Task 37 Orchestrator 500 行治理第十一切片
- **状态：** complete
- 背景：
  - 阶段 237 已把 tool operation façade 迁入 `orchestrator_tool_facade.py`；`orchestrator.py` 仍直接承载 diagnostics / dashboard wrapper，包括 doctor/CLI、Meegle preflight fallback、dashboard payload 和 preflight/source-resolve formatter。
  - 本阶段选择 diagnostics/dashboard façade 作为低风险切片；不迁 Gateway route、tool operation façade、task creation/source helper、runner/workspace/git、`start_run()` 或 run lifecycle。
- 执行的操作：
  - 扩展 `tests/test_architecture_guard.py`，要求 `orchestrator.py` 不再定义 diagnostics/dashboard façade 方法。
  - RED 已确认：focused architecture test 先失败于 `orchestrator.py` 仍定义 10 个 diagnostics/dashboard façade 方法。
  - 新增 `coding_orchestration/orchestrator_diagnostics_facade.py`，用 `OrchestratorDiagnosticsFacadeMixin` 承接 doctor/CLI wrapper、Meegle preflight fallback、dashboard status payload 和 preflight/source-resolve formatter wrapper。
  - `CodingOrchestrator` 改为继承 `OrchestratorDiagnosticsFacadeMixin`，删除主文件内对应 diagnostics/dashboard façade 实现。
- 当前行数：
  - `coding_orchestration/orchestrator.py`：2246 行。
  - `coding_orchestration/orchestrator_diagnostics_facade.py`：80 行。
- 已验证：
  - RED：`rtk proxy python3 -m unittest tests.test_architecture_guard.ArchitectureGuardTest.test_orchestrator_does_not_keep_diagnostics_facade_methods -v` 先失败 10 个 subTest，失败点为 diagnostics/dashboard façade 方法仍在 `orchestrator.py`。
  - Focused GREEN：上述 focused architecture test 重跑通过。
  - 编译检查：`rtk proxy python3 -m py_compile coding_orchestration/orchestrator.py coding_orchestration/orchestrator_diagnostics_facade.py tests/test_architecture_guard.py`：passed。
  - 相邻回归：`rtk proxy python3 -m unittest tests.test_coding_diagnostics_command_executor tests.test_coding_cli tests.test_dashboard_api_contract tests.test_gateway_command_group_flow tests.test_source_resolver -v`：46 tests passed。
  - 文档/架构测试：`rtk proxy python3 -m unittest tests.test_docs_and_install_entry tests.test_architecture_guard -v`：30 tests passed。
  - YAML 解析：`rtk proxy python3 -c 'import yaml; yaml.safe_load(open("contracts/project-context.yaml", encoding="utf-8")); print("yaml ok")'`：passed。
  - 架构检查：`rtk proxy python3 scripts/architecture_guard.py`：passed，仅 watch `coding_orchestration/orchestrator.py: 2246 lines`。
  - 格式检查：`rtk proxy git diff --check`：passed。
  - Release gate no-smoke：`rtk proxy python3 scripts/release_readiness.py --skip-hermes-smoke`：passed，完整单测 981 tests passed，敏感扫描 no findings。
- 剩余风险：
  - 500 行是长期目标；本阶段未迁 Gateway route、tool operation façade、task creation/source helper、active run reconcile、runner/workspace/git、`start_run()`、run lifecycle 或状态机核心。

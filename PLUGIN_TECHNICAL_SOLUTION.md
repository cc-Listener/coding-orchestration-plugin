...
| 2-4 配置 / 工具 / 端口 | Config / Tool / Port contract | Hermes host adapter、MCP adapter、application service | `RuntimeConfig`、`coding_orchestration/config/runtime.py`、`ToolSpec`、`coding_orchestration/ports/contracts.py` 和 contract tests |
...
| Gateway / command | `coding_orchestration/gateway/` + executor host shell | 解析 host event、生成 route metadata、委托 service / façade | controller 写 ledger、启动 runner、发送 Gateway 消息或推进状态 | controller 可纯测试，副作用只在 host shell / application service，Gateway 专属模块不得散落在包根 |
...
| Task 29. Command / Gateway controller 瘦身 | Host shell | Complete | `coding_orchestration/gateway/gateway_command_controller.py` 负责显式 `/coding` / `/commands` 解析、route metadata、handler key、reply mode、task id 来源、确认/授权/dedupe 纯规则；`coding_orchestration/gateway/gateway_command_executor.py` 承接 task/run/delivery/implementation/QA/prepare-merge-test/merge-test custom route host shell；`coding_orchestration/gateway/gateway_pending_action_executor.py` 承接 pending action 确认/取消、latest human_required fallback 和显式命令续接；`coding_orchestration/gateway/gateway_active_context.py` 承接 active project 回填缺项目 task。controller 不承载 ledger/runner/Gateway 发送/状态推进；runner/workspace/git/run lifecycle 不属于 Task 29；目录治理已把 Gateway 专属模块收拢到 `coding_orchestration/gateway/`，包根不得再新增 `gateway_*.py` |
| Task 30. Run orchestration service 闭环 | Run application service | Complete | 已迁出后台等待完成、失败 transition、merge-test pending action、run completion projection、runner/checkpoint failure report projection、diff guard / implementation commit missing blocked report 构造、run report refinement projection、run diff guard observation、runner dispatch、run status transition、run evidence observation、observed run report 构造、stale completion 观测、session/prompt/start selection、run manifest session metadata host writeback、run artifact 文件写入、run artifact path contract、report/summary artifact 读写（现已收拢到 `coding_orchestration/run/artifacts/`）、run projection 纯 payload/规则 helper（现已收拢到 `coding_orchestration/run/projections/`）、run ledger/session/summary writeback callback service（现已收拢到 `coding_orchestration/run/services/`）、run manifest/session policy helper（现已收拢到 `coding_orchestration/run/services/`）、run report summary excerpt、execution policy artifact 读取、completion/report/project writeback payload、Project writeback host gate/callback、summary writer host callback、start_run 与 active run reconcile 的 artifact / agent_run / merge-test record 写回 payload 聚合、run ledger host callback、run session host callback、completed run 与 active run reconcile 的 run summary writer payload 聚合、fresh completed run 写回协调和 active run reconcile 写回协调（现均已收拢到 `coding_orchestration/run/services/`）等规则；`run/services/run_completion_writeback_service.py` 和 `run/services/run_reconcile_writeback_service.py` 分别负责 fresh completed 与 active reconcile 的完成态写回协调。后续大文件治理、hard code 清理和 Hermes/Skill 深度解耦进入 Task 31+ / Task 18/20 长期治理；Task 32 第四切片后 `orchestrator.py` 当前约 4701 行。 |
| Task 31. SourcePort 消费闭环 | Source adapter + Task service | Complete | 已新增 `source/source_projection.py`，把 `SourceResult` / legacy `source_context` 投影为 prompt/context/status/manifest/run-preflight/dashboard/diagnostic payload/task creation helper 可消费的稳定字段；`prompts/source_block.py` 已先消费 `SourceProjection` 输出 `source_status`、恢复动作、deferred/Codex note 和 raw_fields；`run_context_artifact_service.py` 写 `context-index.json` 时已追加稳定 `source_projection` 并保留 legacy `source_context`；`ContextAssembler._current_task_block()` 已通过 `SourceProjection.raw_fields_summary` 输出 `source_summary`；`TaskService.task_status_payload()` / `next_actions_for_task_payload()` 已消费 projection；`run/services/run_manifest_service.py` 已通过 projection 判断 plan-only source elevation；`source/source_context_repair_service.py` / `CodingOrchestrator._enrich_deferred_source_context_before_run()` 已通过 projection 判断 run 前 deferred Feishu source refresh；`CodingOrchestrator.dashboard_status_payload()` 已通过 projection 统计 dashboard source health；`TaskService.source_context_payload()` 已通过 projection 输出 source-resolve / diagnostic payload 稳定字段；`TaskService` task creation helper 已通过 projection 消费 source status、Codex 可读性、title、summary 和 raw_fields。legacy `source_context` 仅保留在 ledger/storage/source reader 兼容边界；source reader/schema 深改不属于 Task 31 |
| Task 32. Tool / MCP operation dispatcher | Tool contract + WorkItem adapter | Complete | 已新增 `ToolOperationDispatcher`，`tools/plugin_tools.py` Hermes native tool 注册层只包装 `ToolSpec.operation_id`，不再维护 `operation_id -> CodingOrchestrator.tool_*` 映射；`cli/registration.py` 中 `lark-preflight` / `source-resolve` / `project-mcp-preflight` / 普通 `status <task_id>` 已接入同一 dispatcher，其中 `project-mcp-preflight` 保留 host config 与 stdio command readiness gate，`status <task_id>` 只读取 `task.status` payload；Gateway `/coding project-mcp-preflight` 已补成 diagnostic immediate reply 并复用同一 preflight façade；future MCP tool 只有出现真实消费方后再接入，不作为当前未完成项。共享 dispatcher 不下沉 `doctor` 聚合、Gateway status、delivery/tree presentation、active run reconcile 或 runner/workspace/git/run lifecycle |
| Task 33. Skill 零耦合复查 | Skill contract | Complete | `coding-operator-core` / `coding-health-core` 保持 host-agnostic；`hermes-coding-operator` 和 `hermes-coding-health-check` 只保留 core 引用、Hermes `/coding` / native tools / doctor/preflight / 配置引用映射，不再复制通用 playbook、readiness 输出格式或内部状态规则 |
| Task 34. Orchestrator façade 降载 | Architecture guard | Complete | 已新增 `gateway_project_context.py`，迁出本地项目候选、搜索根、路径解析和人工别名抽取纯规则；已新增 `services/merge_test_readiness_service.py`，迁出 blocked task 是否可继续 merge-test 的纯评估规则；已新增 `project/project_profile_catalog.py`，迁出 project profile 读取/格式化、registry fallback、别名查找和动态来源计数；已新增 `gateway_rewrite_context.py`，迁出 Coding Mode rewrite context 投影和 task next-step hint 纯规则；已新增 `commands/delivery/delivery_command_executor.py`，迁出 delivery command façade；已新增 `commands/project/project_command_executor.py`，迁出 `/coding project list/init/use/status/clear` 命令模式提示和 Gateway immediate project context host shell；已新增 `coding_diagnostics_command_executor.py`，迁出 doctor / lark-preflight / project-mcp-preflight / source-resolve diagnostic host shell；已新增 `coding_status_command_executor.py`，迁出普通 status 与 Gateway status host shell；已新增 `coding_task_list_command_executor.py`，迁出普通 list 与 Gateway list host shell；已新增 `coding_run_command_executor.py`，迁出普通 run / implement / QA command-mode host shell；已新增 `coding_merge_test_command_executor.py`，迁出普通 prepare/merge-test command host shell；已新增 `coding_feedback_command_executor.py`，迁出 continue/change/bugfix feedback host shell 和记录 helper；已新增 `coding_task_control_command_executor.py`，迁出普通 use/exit/cancel/restore/delete/complete task control host shell 和 Gateway use/exit task binding shell；已新增 `gateway_coding_mode_executor.py`，迁出 Gateway coding mode enter/exit、pending rewrite、rewrite direct execute/handoff 和 context 装配 host shell；已新增 `coding_help_command_executor.py`，迁出普通 help、Gateway commands listing 和 Hermes built-in command list 拼接 host shell；已新增 `background/coding_background_run_executor.py`，迁出 Gateway 后台 plan/implementation/QA/merge-test mode-specific 启动和 completion notification 接线；已新增 `source_context_repair_service.py`，迁出 source context 读取、deferred source pre-run enrichment 和既有 task context 修复 host helper；已新增 `kanban_sync_service.py`，迁出 Kanban create/status sync host helper 和 sync record 投影；已新增 `services/task_lifecycle_guard_service.py`，迁出取消态保护、恢复状态选择和 merged_test bugfix reopen helper；`orchestrator.py` 已低于 Task 34 的 3000 行退出信号，后续 500 行目标进入 Task 37；目录治理已将普通 coding command executor 收拢到 `coding_orchestration/coding_commands/`，将 delivery/project command host shell 收拢到 `coding_orchestration/commands/`，将 Feishu/Lark 文案、reader 与 Project MCP adapter 收拢到 `coding_orchestration/feishu/`，并将 doctor/run start/run completion/feedback/merge-test/task list/task status presenter 收拢到 `coding_orchestration/presenters/`，将 project domain 模块收拢到 `coding_orchestration/project/` |
| Task 35. Legacy test final cleanup | Test governance | Complete | 旧私有 helper / 旧文件形态测试已全部有保留、改写或删除记录：`tests/test_orchestrator_run_flow.py` 只保留端到端 smoke、Kanban transition 和 plan-only resume sandbox；delivery/source/QA/completion/cancel/merge-test/plan/implementation/Gateway/status 等旧文件形态覆盖已迁到行为域小文件；service/policy/presenter/adapter contract tests 承接旧私有 helper 断言；`tests/test_architecture_guard.py` 已锁定不再给 resolved legacy flow suite 保留 large-file 豁免；阶段 264 已将包根模块族目录表拆到 `tests/test_architecture_module_layout.py`，并用 `tests/test_architecture_guard_test_governance.py` 锁定主 guard 测试文件增长缓冲。 |
| Task 36. Release readiness and operating contract | 文档合同 + 运行治理 | Complete | 已新增 `scripts/release_readiness.py` 作为发布 gate：默认顺序执行完整单测、architecture guard、`git diff --check`、敏感扫描和最小 Hermes smoke（`hermes plugins list`、`hermes gateway status`、Gateway health）；本机未运行 Hermes 时可显式 `--skip-hermes-smoke` 跳过 smoke，但正式发布前仍需要补跑。 |
| Task 37. Orchestrator 500-line façade governance | Architecture guard + host shell cleanup | Complete | 长期目标是将旧包根 `orchestrator.py` 缩到 500 行左右并最终包化到 `coding_orchestration/orchestrator/facade.py`，只保留注册/host façade/少量兼容入口；第一切片已让 `coding_run_command_executor.py`、`coding_merge_test_command_executor.py` 和 `background/coding_background_run_executor.py` 直接消费 `run_completion_presenter.py`，并删除 `CodingOrchestrator._format_*completion_message` / completion helper 代理；第二切片已让 `gateway_command_executor.py`、`coding_run_command_executor.py`、`gateway_coding_mode_executor.py` 和 `RunService` 直接消费 `run_start_presenter.py`，并删除 run start 文案代理；第三切片让 `coding_feedback_command_executor.py` 直接消费 `feedback_presenter.py`，并删除 feedback 文案代理；第四切片让 `coding_merge_test_command_executor.py` 和 `gateway_command_executor.py` 直接消费 `merge_test_presenter.py`，并删除 merge-test 文案代理；第五切片删除 doctor/preflight presenter 私有代理，doctor、lark-preflight、project-mcp-preflight 和 source-resolve 文案继续由 `doctor_presenter.py` owner 负责，并由 `coding_diagnostics_command_executor.py` 直接消费；第六切片删除 Gateway rewrite presenter 私有代理，Coding Mode rewrite 确认、低置信度补充和 handoff 文案继续由 `gateway_rewrite_presenter.py` owner 负责，并由 `gateway_coding_mode_executor.py` 直接消费；第七切片删除 task list presenter 私有代理，`/coding list` 任务摘要、项目标签和描述摘要继续由 `task_list_presenter.py` owner 负责，并由 `coding_task_list_command_executor.py` / `gateway_coding_mode_executor.py` 直接消费；第八切片删除 task status presenter 展示/QA helper 私有代理，`/coding status` 详情、Kanban/完成回传和 QA 缺口展示继续由 `task_status_presenter.py` owner 负责，并由 `coding_status_command_executor.py` direct 消费，`_read_report_json` 作为跨模块 host callback 暂保留；第九切片新增 `orchestrator_facades/orchestrator_command_facade.py` mixin，承接普通 `/coding ...` command façade 分发表、command wrapper、delivery rollup/materialize helper、artifact purge callback 和 prepare-merge-test status callback；第十切片新增 `orchestrator_facades/orchestrator_tool_facade.py` mixin，承接 tool operation dispatcher façade、`tool_*` 入口、tool run/source preflight adapter 和 Project MCP / WorkItem 小包装；第十一切片新增 `orchestrator_facades/orchestrator_diagnostics_facade.py` mixin，承接 doctor/CLI wrapper、Meegle preflight fallback、dashboard status payload 和 preflight/source-resolve formatter wrapper；第十二切片新增 `orchestrator_facades/orchestrator_gateway_facade.py` mixin，承接 Gateway immediate/custom route dispatch、Coding Mode rewrite wrapper、pending action/rewrite binding wrapper、确认/取消词 wrapper、commands listing route、dedupe/授权/looks-like-task wrapper；第十三切片新增 `orchestrator_facades/orchestrator_project_facade.py` mixin，承接 project profile catalog wrapper、active project binding wrapper、本地项目候选/路径/别名 wrapper、active project 回填 wrapper、人工项目画像 upsert 和项目澄清 host callback；第十四切片新增 `orchestrator_facades/orchestrator_task_source_facade.py` mixin，承接 task create command wrapper、TaskService source/status payload wrapper、source context 读取/修复/enrich wrapper、event/media ledger helper 和 draft knowledge source refs wrapper；第十五切片新增 `orchestrator_facades/orchestrator_task_runtime_facade.py` mixin，承接 Kanban/status host wrapper、task list/status event wrapper、feedback active-task wrapper、active task/coding mode binding wrapper、task lifecycle wrapper、人工决策记录和 RunService gate wrapper；第十六切片新增 `orchestrator_facades/orchestrator_workspace_facade.py` mixin，承接 implementation/merge-test workspace 查找、QA artifact 收集、QA/merge-test checkpoint 准备、dirty/head/clean checkpoint 观测、source branch/base branch 投影、task short id/slug 和 latest existing implementation workspace wrapper；第十七切片新增 `orchestrator_facades/orchestrator_background_facade.py` mixin，承接后台 plan/implementation/QA/merge-test 启动与通知 wrapper、completion wait/failed/pending-action/notification wrapper、sender scheduling/reply wrapper 和 execution policy artifact wrapper；第十八切片新增 `orchestrator_facades/orchestrator_status_policy_facade.py` mixin，承接 report status details、implementation status normalize、verification limitations 补齐和 run result 到 task status/phase 映射 wrapper；第十九切片新增 `orchestrator_facades/orchestrator_manifest_facade.py` mixin，承接 existing/fresh artifact set、run manifest 构造、Codex attach/resume 命令、session id 读取、runner manifest name 和 manifest session metadata 写回 wrapper；第二十切片新增 `orchestrator_facades/orchestrator_prompt_context_facade.py` mixin，承接 resumed session 增量上下文、wiki docs/refs、context artifacts、dependency/sibling task context、confirmed plan / merge-test context、report summary markdown 和 wiki ref 投影 wrapper；第二十一切片新增 `orchestrator_facades/orchestrator_merge_test_facade.py` mixin，承接 merge-test blocker、blocked task 风险放行 host 写回、blocked assessment fact collection、latest implementation run/source branch/disallowed reason wrapper 和 QA evidence wrapper；第二十二切片新增 `orchestrator_facades/orchestrator_runtime_facade.py` mixin，承接 flag helper、project/workflow wrapper、plan report session/execution policy/timeout wrapper、runner failed result 和 checkpoint failed result；第二十三切片新增 `orchestrator_facades/orchestrator_active_run_facade.py` mixin，承接 completed active run reconcile host shell 和 agent run lookup helper；第二十四切片新增 `orchestrator_facades/orchestrator_bootstrap_facade.py` mixin，承接 `__post_init__`、`set_dispatch_tool`、`from_default_config` 和默认运行根 helper。当前 façade 已包化到 `coding_orchestration/orchestrator/facade.py`，500 行治理目标与包根收尾均达成；后续目录治理已把上述 façade mixin 收拢到 `coding_orchestration/orchestrator_facades/`，Gateway 专属模块收拢到 `coding_orchestration/gateway/`，普通 coding command executor 收拢到 `coding_orchestration/coding_commands/`，presenter 收拢到 `coding_orchestration/presenters/`，prompt/context helper 收拢到 `coding_orchestration/prompting/`，并由架构测试锁定包根不再散落 `orchestrator_*_facade.py`、`gateway_*.py`、`coding_*_command_executor.py`、`*_presenter.py`、`prompt_builder.py`、`context_assembler.py` 或 `pre_llm_context.py`；更深层 run lifecycle / workspace / runner 解耦继续作为后续独立任务按边界推进。 |
...
阶段 285 目录治理事实：Tool contract/dispatcher 已收拢到 `coding_orchestration/tools/`，包括 `tool_specs.py` 和 `tool_operation_dispatcher.py`；该包只承接 Hermes native tools / future MCP tools 的 host-agnostic 规格合同和 `ToolSpec.operation_id` 分发，当时不迁 `plugin_tools.py` host 注册入口、CLI/Gateway diagnostic host shell、orchestrator tool façade、WorkItem/Task service、runner/workspace/git 或 run lifecycle；`plugin_tools.py` 已在阶段 290 继续收拢。

阶段 290 目录治理事实：Plugin tool registration 已收拢到 `coding_orchestration/tools/plugin_tools.py`；该模块只承接 Hermes native tool host 注册、ToolSpec schema 包装、operation dispatcher handler 绑定和 legacy `register_tool` fallback，不迁 CLI、orchestrator tool façade、ToolSpec/dispatcher 纯合同、WorkItem/Task service、runner/workspace/git 或 run lifecycle。
...
阶段 294 目录治理事实：Port contract 已收拢到 `coding_orchestration/ports/contracts.py`，并通过 `coding_orchestration/ports/__init__.py` 保留 `HostPort`、`RunnerPort`、`SourcePort`、`SourceResult`、`WorkItemPort`、`LedgerPort`、`KnowledgePort`、`NotifierPort`、`RuntimePort` 和 `RunServicePort` 稳定导出；该模块只承接 runtime-checkable protocol 与 `SourceResult` 状态归一，不迁具体 Hermes/MCP/Codex/SQLite/Knowledge adapter、source resolver、runner/workspace/git 或 run lifecycle。
...
阶段 299 目录治理事实：Orchestrator façade 已收拢到 `coding_orchestration/orchestrator/facade.py`，并通过 `coding_orchestration/orchestrator/__init__.py` 保留 `from coding_orchestration.orchestrator import CodingOrchestrator` 与测试 patch target 所需的 `source_projection` 稳定导出；该包只承接 Gateway 事件入口、pre-LLM context、`start_run()` 主体和 façade mixin 组合，不迁 runner/workspace/git、artifact/service 副作用或 run lifecycle。阶段完成后 `coding_orchestration/` 包根应用 `.py` 清零，仅保留包入口 `__init__.py`。
...
阶段 276 目录治理事实：Completed run writeback coordinator 已收拢到 `coding_orchestration/run/services/run_completion_writeback_service.py`；该模块继续维护 fresh completed run 的 completion projection、stale observation、状态 transition、ledger/session/summary/project writeback 和 final result payload 协调，不迁 active reconcile、run manifest、run orchestration projection、runner/workspace/git 或 run lifecycle。

阶段 277 目录治理事实：Active run reconcile writeback coordinator 已收拢到 `coding_orchestration/run/services/run_reconcile_writeback_service.py`；该模块继续维护 active run reconcile 完成态的 completion projection、report finalization、状态 transition、ledger upsert、runner session update、summary writer callback 和 result payload 协调，不迁 fresh completion、run manifest、run orchestration projection、runner/workspace/git 或 run lifecycle。

阶段 278 目录治理事实：Run manifest/session policy helper 已收拢到 `coding_orchestration/run/services/run_manifest_service.py`；该模块继续维护 run-manifest 基础字段、启动期 manifest update projection、artifact record、Codex attach/resume 展示命令、manifest session metadata 字段投影、controlled bypass 权限 profile 和 source elevated plan 权限判断，不迁 run orchestration projection、artifact 文件写入、runner/workspace/git 或 run lifecycle。
...
阶段 270 目录治理事实：`coding_orchestration/integrations/install/` 已收拢安装/卸载逻辑；脚本仍只保留入口和用户输出，install integration 只维护 preflight、symlink install、legacy/current uninstall dry-run/execute，不写入 auth/token/.env 或本地运行根内容。

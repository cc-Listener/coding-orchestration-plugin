...
| Task 34. Orchestrator façade 降载 | Architecture guard | In Progress | 已新增 `gateway_project_context.py`，迁出本地项目候选、搜索根、路径解析和人工别名抽取纯规则；已新增 `merge_test_readiness_service.py`，迁出 blocked task 是否可继续 merge-test 的纯评估规则；`orchestrator.py` 当前约 4523 行，后续继续迁 project profile 读取/格式化、delivery command façade 和 rewrite context |
...
- `orchestrator.py` 目前仍是唯一核心大文件债务，当前约 4523 行；Task 30 已关闭，短期目标是继续通过 Task 31+ / Task 34 进入 3000 行以内的 façade 降载。

...
- `orchestrator.py` 目前仍是唯一核心大文件债务，当前约 4793 行；短期目标是继续迁出 run orchestration 副作用，先降到 3000 行以内。

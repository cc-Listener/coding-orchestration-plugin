...
| Task 35. Legacy test final cleanup | Test governance | Complete | 旧私有 helper / 旧文件形态测试已全部有保留、改写或删除记录：`tests/test_orchestrator_run_flow.py` 只保留端到端 smoke、Kanban transition 和 plan-only resume sandbox；delivery/source/QA/completion/cancel/merge-test/plan/implementation/Gateway/status 等旧文件形态覆盖已迁到行为域小文件；service/policy/presenter/adapter contract tests 承接旧私有 helper 断言；`tests/test_architecture_guard.py` 已锁定不再给 resolved legacy flow suite 保留 large-file 豁免。 |

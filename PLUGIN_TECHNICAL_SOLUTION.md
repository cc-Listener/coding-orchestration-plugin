...
| Task 34. Orchestrator façade 降载 | Architecture guard | In Progress | 已新增 `gateway_project_context.py`，迁出本地项目候选、搜索根、路径解析和人工别名抽取纯规则；已新增 `merge_test_readiness_service.py`，迁出 blocked task 是否可继续 merge-test 的纯评估规则；已新增 `project_profile_catalog.py`，迁出 project profile 读取/格式化、registry fallback、别名查找和动态来源计数；已新增 `gateway_rewrite_context.py`，迁出 Coding Mode rewrite context 投影和 task next-step hint 纯规则；已新增 `delivery_command_executor.py`，迁出 delivery command façade；已新增 `project_command_executor.py`，迁出 `/coding project list/init/use/status/clear` 命令模式提示和 Gateway immediate project context host shell；已新增 `coding_diagnostics_command_executor.py`，迁出 doctor / lark-preflight / project-mcp-preflight / source-resolve diagnostic host shell；已新增 `coding_status_command_executor.py`，迁出普通 status 与 Gateway status host shell；已新增 `coding_task_list_command_executor.py`，迁出普通 list 与 Gateway list host shell；已新增 `coding_run_command_executor.py`，迁出普通 run / implement / QA command-mode host shell；已新增 `coding_feedback_command_executor.py`，迁出 continue/change/bugfix feedback host shell 和记录 helper；已新增 `coding_task_control_command_executor.py`，迁出普通 use/exit/cancel/restore/delete/complete task control host shell 和 Gateway use/exit task binding shell；`orchestrator.py` 当前约 3668 行，后续继续选择 prepare/merge-test command host shell、Gateway 执行副作用或其他非 runner/workspace/git façade 降载点 |

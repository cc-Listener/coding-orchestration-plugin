...
| Task 34. Orchestrator façade 降载 | Architecture guard | Complete | 已新增 `gateway_project_context.py`，迁出本地项目候选、搜索根、路径解析和人工别名抽取纯规则；已新增 `services/merge_test_readiness_service.py`，迁出 blocked task 是否可继续 merge-test 的纯评估规则；已新增 `project/project_profile_catalog.py`，迁出 project profile 读取/格式化、registry fallback、别名查找和动态来源计数；已新增 `gateway_rewrite_context.py`，迁出 Coding Mode rewrite context 投影和 task next-step hint 纯规则；已新增 `commands/delivery/delivery_command_executor.py`，迁出 delivery command façade；已新增 `commands/project/project_command_executor.py`，迁出 `/coding project list/init/use/status/clear` 命令模式提示和 Gateway immediate project context host shell；已新增 `coding_diagnostics_command_executor.py`，迁出 doctor / lark-preflight / project-mcp-preflight / source-resolve diagnostic host shell；已新增 `coding_status_command_executor.py`，迁出普通 status 与 Gateway status host shell；已新增 `coding_task_list_command_executor.py`，迁出普通 list 与 Gateway list host shell；已新增 `coding_run_command_executor.py`，迁出普通 run / implement / QA command-mode host shell；已新增 `coding_merge_test_command_executor.py`，迁出普通 prepare/merge-test command host shell；已新增 `coding_feedback_command_executor.py`，迁出 continue/change/bugfix feedback host shell 和记录 helper；已新增 `coding_task_control_command_executor.py`，迁出普通 use/exit/cancel/restore/delete/complete task control host shell 和 Gateway use/exit task binding shell；已新增 `gateway_coding_mode_executor.py`，迁出 Gateway coding mode enter/exit、pending rewrite、rewrite direct execute/handoff 和 context 装配 host shell；已新增 `coding_help_command_executor.py`，迁出普通 help、Gateway commands listing 和 Hermes built-in command list 拼接 host shell；已新增 `coding_background_run_executor.py`，迁出 Gateway 后台 plan/implementation/QA/merge-test mode-specific 启动和 completion notification 接线；已新增 `source_context_repair_service.py`，迁出 source context 读取、deferred source pre-run enrichment 和既有 task context 修复 host helper；已新增 `kanban_sync_service.py`，迁出 Kanban create/status sync host helper 和 sync record 投影；已新增 `services/task_lifecycle_guard_service.py`，迁出取消态保护、恢复状态选择和 merged_test bugfix reopen helper；`orchestrator.py` 已低于 Task 34 的 3000 行退出信号，后续 500 行目标进入 Task 37；目录治理已将普通 coding command executor 收拢到 `coding_orchestration/coding_commands/`，将 delivery/project command host shell 收拢到 `coding_orchestration/commands/`，将 Feishu/Lark 文案、reader 与 Project MCP adapter 收拢到 `coding_orchestration/feishu/`，并将 doctor/run start/run completion/feedback/merge-test/task list/task status presenter 收拢到 `coding_orchestration/presenters/`，将 project domain 模块收拢到 `coding_orchestration/project/` |

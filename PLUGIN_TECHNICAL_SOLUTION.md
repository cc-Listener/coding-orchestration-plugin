...
| Task 29. Command / Gateway controller 瘦身 | Host shell | Complete | `gateway_command_controller.py` 负责显式 `/coding` / `/commands` 解析、route metadata、handler key、reply mode、task id 来源、确认/授权/dedupe 纯规则；`gateway_command_executor.py` 承接 task/run/delivery/implementation/QA/prepare-merge-test/merge-test custom route host shell；`gateway_pending_action_executor.py` 承接 pending action 确认/取消、latest human_required fallback 和显式命令续接；`gateway_active_context.py` 承接 active project 回填缺项目 task。controller 不承载 ledger/runner/Gateway 发送/状态推进；runner/workspace/git/run lifecycle 不属于 Task 29，后续进入独立任务治理 |

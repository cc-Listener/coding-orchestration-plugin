...
| Task 32. Tool / MCP operation dispatcher | Tool contract + WorkItem adapter | In Progress | 已新增 `ToolOperationDispatcher`，Hermes native tool 注册层只包装 `ToolSpec.operation_id`，不再维护 `operation_id -> CodingOrchestrator.tool_*` 映射；CLI `lark-preflight` / `source-resolve` / `project-mcp-preflight` 已接入同一 dispatcher，其中 `project-mcp-preflight` 保留 host config 与 stdio command readiness gate；Gateway `/coding project-mcp-preflight` 已补成 diagnostic immediate reply 并复用同一 preflight façade；后续继续评估 CLI `status <task_id>` 读取 `task.status` dispatcher 和 future MCP tool，共享 dispatcher 不应下沉 `doctor` 聚合或 `status` presentation |

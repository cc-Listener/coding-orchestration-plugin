...
- `orchestrator.py` 目前仍是唯一核心大文件债务，当前约 3813 行；Task 30 已关闭，短期目标是继续通过 Task 31+ / Task 34 进入 3000 行以内的 façade 降载。

...
| Task 32. Tool / MCP operation dispatcher | Tool contract + WorkItem adapter | In Progress | 已新增 `ToolOperationDispatcher`，Hermes native tool 注册层只包装 `ToolSpec.operation_id`，不再维护 `operation_id -> CodingOrchestrator.tool_*` 映射；CLI `lark-preflight` / `source-resolve` 已接入同一 dispatcher，后续继续评估 `project-mcp-preflight`、`status` 和 future MCP tool 共用 dispatcher |

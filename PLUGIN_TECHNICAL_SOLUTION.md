...
| Task 31. SourcePort 消费闭环 | Source adapter + Task service | In Progress | 已新增 `source_projection.py`，把 `SourceResult` / legacy `source_context` 投影为 prompt/context/status/manifest/run-preflight/dashboard/diagnostic payload 可消费的稳定字段；`prompts/source_block.py` 已先消费 `SourceProjection` 输出 `source_status`、恢复动作、deferred/Codex note 和 raw_fields；`run_context_artifact_service.py` 写 `context-index.json` 时已追加稳定 `source_projection` 并保留 legacy `source_context`；`ContextAssembler._current_task_block()` 已通过 `SourceProjection.raw_fields_summary` 输出 `source_summary`；`TaskService.task_status_payload()` / `next_actions_for_task_payload()` 已消费 projection；`run_manifest_service.source_requires_codex_plan_permissions()` 已通过 projection 判断 plan-only source elevation；`CodingOrchestrator._enrich_deferred_source_context_before_run()` 已通过 projection 判断 run 前 deferred Feishu source refresh；`CodingOrchestrator.dashboard_status_payload()` 已通过 projection 统计 dashboard source health；`TaskService.source_context_payload()` 已通过 projection 输出 source-resolve / diagnostic payload 稳定字段。ledger 中 legacy `source_context` 仍保留；task creation source helper 仍可后续单独小切片收口 |

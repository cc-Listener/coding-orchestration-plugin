...
| Task 31. SourcePort 消费闭环 | Source adapter + Task service | In Progress | 已新增 `source_projection.py`，把 `SourceResult` / legacy `source_context` 投影为 prompt/context 可消费的稳定字段；`prompts/source_block.py` 已先消费 `SourceProjection` 输出 `source_status`、恢复动作、deferred/Codex note 和 raw_fields；`run_context_artifact_service.py` 写 `context-index.json` 时已追加稳定 `source_projection` 并保留 legacy `source_context`；`ContextAssembler._current_task_block()` 已通过 `SourceProjection.raw_fields_summary` 输出 `source_summary`。ledger 中 legacy `source_context` 仍保留；TaskService status payload/next actions、manifest 权限判断和 orchestrator enrichment 仍是后续切片 |

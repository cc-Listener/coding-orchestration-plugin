...
需求来自飞书 Project、Wiki、Doc 或 Docx 链接时，Hermes 先通过 Gateway / Feishu reader 尝试读取正文；文档兜底读取由 source enrichment 层调用 `rtk lark-cli docs +fetch ...`，不放到 Codex runner 里。读取成功时，把正文摘要写入来源上下文并交给 Codex plan-only 使用；读取失败时，不阻断 task 创建，只索引来源 URL、token、Project key、工作项类型、错误和恢复动作。启动 plan 前，Hermes 会对 failed/indexed/deferred 飞书来源再重试一次 enrichment。项目识别仍优先走 LLM Wiki `project_profile`，缺失时再从文本中的项目名、文件夹名或路径定位本地项目并初始化项目画像。
...
Codex plan-only 不再被要求自行绑定 `lark-cli`。如果没有正文且需求依赖文档，应在 `report.json` 中返回结构化 blocked，说明需要授权 Hermes/Feishu reader、补充可访问内容或直接粘贴来源正文。
...
如果历史安装副本仍在 `~/.hermes/plugins/` 下，需要先移除或停用，保证 discovery 只看到 `coding_orchestration` 这个软链接入口。
...
- `plan-only` 使用 `plan_read_only` 权限 profile，Codex CLI 以只读沙箱运行，只做规划不改项目文件；Hermes 创建 task 时识别项目，并尝试通过 Gateway / Feishu reader 读取飞书 Project/Wiki/Docx。读取成功时注入正文摘要；读取失败时不因权限问题阻断 task，只记录 URL、token、错误和恢复动作。Codex plan-only 优先使用已注入正文；没有正文时不要求自行绑定 `lark-cli`，而是结构化说明需要授权 Hermes/Feishu reader、补充可访问内容或粘贴正文。

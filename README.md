...
完整部署、前置环境检查和排障流程见 [docs/deployment.md](docs/deployment.md)。
...
当飞书消息里包含 `project.feishu.cn/<project_key>/<type>/detail/<id>`、`/wiki/<token>` 或 `/docx/<token>` 链接时，插件会在 Hermes source enrichment 层统一通过 `lark-cli` 读取正文，不交给 Codex runner 自行读取。读取成功时，正文摘要会写入 Task Ledger、LLM Wiki draft 和 Codex prompt；读取失败时，不阻断创建 task，只索引 URL、token、错误和恢复动作。后续 `/coding run <task_id>` 启动 plan 前会再做一次非阻塞 source enrichment，用户补齐授权后无需重建 task。

Codex plan-only 优先使用 Hermes 已注入的正文。若只有链接没有正文，Codex 不应假设文档内容，也不再要求自己绑定 `lark-cli`；需求依赖文档时应返回结构化 blocked，恢复动作是修复 Hermes 环境里的 `lark-cli` 绑定/授权、补充可访问内容，或直接粘贴需求正文。

飞书 Project 链接读取成功时注入工作项摘要；读取失败时保留 project key、工作项类型、工作项 id 和 URL，并继续推进 task 到 plan，让 blocked 的恢复动作指向 Hermes `lark-cli` 授权修复或人工补充正文。
...
plan-only 使用 `plan_read_only` 权限 profile：Codex CLI 以只读沙箱运行，只产出计划，不允许修改项目文件。Hermes 创建 task 时会识别项目，并在 source enrichment 层统一通过 `lark-cli` 读取飞书 Project/Wiki/Docx；读取成功就把正文注入来源上下文，读取失败也不因权限问题阻断 task，而是记录链接、token、错误和恢复动作。启动 plan 前，Hermes 会对 failed/indexed/deferred 飞书来源再重试一次 enrichment。Codex plan-only 优先使用已注入正文；没有正文时不要求自行绑定 `lark-cli`，而是结构化说明需要修复 Hermes `lark-cli` 绑定、补充可访问内容或粘贴正文。implementation 和 QA run 使用受控高权限 Codex CLI session，以便自动安装依赖、访问私有源、启动测试/dev server、执行浏览器 QA、写入 `.gstack` QA 产物，并提交 QA 修复。边界不是放给 runner 自由发挥：子进程 cwd 仍是 task worktree，源码修改只允许落在当前 workspace；项目外写入只限依赖缓存、git metadata、dev server/browser 临时文件和 QA artifact；Hermes 继续用 diff guard 审计 workspace 内改动。

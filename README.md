...
生产环境或团队成员集成时，统一使用 SSH Git URL 安装。安装前先确认部署机能访问仓库：
...
当飞书消息里包含 `project.feishu.cn/<project_key>/<type>/detail/<id>`、`/wiki/<token>` 或 `/docx/<token>` 链接时，插件只在创建 task 时索引来源链接，不再由 Hermes 读取飞书内容，也不要求配置 Hermes 侧 Project/Doc token。

索引会写入 Task Ledger、LLM Wiki draft 和 Codex prompt。Codex plan-only session 负责按来源链接自行读取文档；飞书 Wiki/Doc/Docx 会带上 `lark-cli docs +fetch` 建议命令。若 Codex 无法读取，应在 report 中返回结构化 blocked 和恢复动作，例如绑定 `lark-cli` 或要求用户粘贴文档内容。

飞书 Project 链接也只记录 project key、工作项类型、工作项 id 和 URL，不再要求 Hermes 配置 Project token。Codex 如果无法访问飞书来源，必须在 plan report 中说明原因、影响和恢复动作。
...
plan-only 使用 `plan_read_only` 权限 profile：Codex CLI 以只读沙箱运行，只产出计划，不允许修改项目文件。Hermes 创建 task 时只索引飞书 Project/Wiki/Docx 链接、Swagger/OpenAPI、私有 API 元数据等外部来源，不预读飞书内容、不因飞书权限失败阻断 task；飞书 Wiki/Doc/Docx 链接会把 URL、token 和 `lark-cli docs +fetch` 建议命令写入 source context，由 Codex 在 plan-only session 内自行调用 `rtk lark-cli` 读取。Codex 自己读取仍失败时，才在 report 中结构化返回权限绑定、补充内容或凭证恢复动作。implementation 和 QA run 使用受控高权限 Codex CLI session，以便自动安装依赖、访问私有源、启动测试/dev server、执行浏览器 QA、写入 `.gstack` QA 产物，并提交 QA 修复。边界不是放给 runner 自由发挥：子进程 cwd 仍是 task worktree，源码修改只允许落在当前 workspace；项目外写入只限依赖缓存、git metadata、dev server/browser 临时文件和 QA artifact；Hermes 继续用 diff guard 审计 workspace 内改动。

...
Hermes 主 agent 已有 `coding_task_create`、`coding_task_status`、`coding_task_run`、`coding_source_resolve`、`coding_lark_preflight` native tools；低置信度、纯 source/auth 诊断、权限排查或结构化参数更适合交给主 agent 调用 tools。
...
19. 不要把所有包含 Lark、飞书、Meegle、source、授权、scope、needs_refresh 的消息都降级。
    - 如果用户是在排查授权或来源读取问题，例如“lark-cli 授权失败怎么处理”“为什么读不到飞书文档”“再试一次读取文档”，且没有新的开发需求，输出 `canonical_command=null`；主 agent 应调用 `coding_lark_preflight` 或 `coding_source_resolve`。
    - 如果用户同时给出明确项目/文件夹/active_project 和新的开发需求，即使包含飞书 Wiki/Docx/Meegle 链接、source、需求来源或权限词，也应高置信度输出 `/coding task <原需求> --project <项目名或文件夹>`。Hermes 不预读来源；飞书正文交给 Codex plan 阶段用 `rtk lark-cli` 读取，读取失败再由 Codex 给结构化恢复方案。
20. 不要因为外部来源可能读取失败而阻止创建明确项目的新 task；只有纯授权诊断、纯 source 解析、缺项目或缺需求时才返回 unknown。

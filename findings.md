...
| Run report artifact 写回需要独立 service | `report.json` 落盘是 run artifact 文件写入边界，不应继续散落在 `start_run()`、active run reconcile 或 pre-run failure 路径；`run_report_artifact_service.py` 只写指定 report artifact，不写 manifest/summary/ledger，不启动 runner、不推进状态 |

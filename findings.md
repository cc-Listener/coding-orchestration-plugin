...
| Run report artifact 读写需要独立 service | `report.json` 读写是 run artifact 文件边界，不应继续散落在 `start_run()`、active run reconcile 或 pre-run failure 路径；`run_report_artifact_service.py` 只读写指定 report artifact，缺失或无效读取返回空 dict，不写 manifest/summary/ledger，不启动 runner、不推进状态 |

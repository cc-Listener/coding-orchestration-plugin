...
| `coding_project_mcp_preflight` | 检查插件私有飞书项目 MCP 配置、transport 和 `MCP_USER_TOKEN` |
...
飞书项目 Story / Issue / WBS / 状态流转读写通过插件内私有 `FeishuProjectMcpAdapter` 完成。插件只读取 `~/.hermes/coding-orchestration/mcp.json` 中的 `mcpServers.feishu-project` 配置，并负责 MCP transport、工具白名单、写操作确认、审计和脱敏；Codex / Claude / Gemini runner 不直接配置飞书项目 MCP，不持有 `MCP_USER_TOKEN`，也不直接写飞书项目。
...
飞书项目 MCP 是可选能力，不是 `install_symlink.py` 的安装硬门禁。需要启用自动 intake、WBS 更新、状态流转或 bugfix intake 时，先配置 `~/.hermes/coding-orchestration/mcp.json` 并运行：

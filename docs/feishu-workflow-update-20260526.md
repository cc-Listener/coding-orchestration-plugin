...
Hermes 负责流程和策略，Runner 负责执行，人负责关键判断。

用户在飞书里提需求。Hermes 判断这是不是一个编码任务，补齐项目和上下文，再按任务难度和风险生成 Execution Policy：简单任务可以跳过 plan-only 直接进入实现，标准需求走 plan-only -> implementation，受保护变更会展开 reviewed plan -> implementation -> full QA -> merge-test 的完整链路。Runner 每次执行后的日志、diff、测试结果和报告都会回到 Hermes，任务状态写入 Task Ledger，有复用价值的经验写入 LLM Wiki。

| 角色 | 做什么 | 不做什么 |
| --- | --- | --- |
| 飞书 | 提需求、补反馈、确认关键节点 | 不直接驱动代码执行 |
| Hermes | 判断意图、建 task、选执行链路、管状态、发执行包、收证据 | 不亲自写业务代码 |
...
4. **选择执行链路**：Hermes 根据 Execution Policy 决定走 inline implementation、标准 plan-only 链路，还是高风险 reviewed plan 链路。
...
最新版本不是把所有任务都固定跑完整链路，而是先判断任务难度和风险，再选择不同深度的 RunMode pipeline。

| 任务类型 | 触发线索 | Execution Policy | RunMode 链路 | 典型 task status |
| --- | --- | --- | --- | --- |
| 快速修复 | `.gitignore`、`.gstack`、简单修、小问题 | `route=fast_fix`，`planning=inline`，`verification=targeted` | `implementation -> merge-test` | `running -> ready_for_merge_test -> merged_test -> done` |
| 小型 UI 行为修复 | 复制、文案、标题、链接、tooltip 等局部行为 | `route=targeted_ui_fix`，`planning=inline`，`context=focused` | `implementation -> merge-test`，按需补 QA | `running -> ready_for_merge_test -> merged_test -> done` |
...
| `blocked` | `blocked`，但 implementation 有有效改动且带 known gaps 时可投影到 `ready_for_merge_test` 并要求风险确认 |
| `failed` | `failed` |
| `cancelled` | `cancelled` |

旧状态例如 `success`、`queued`、`timeout`、`runner_failed`、`ready_for_merge_test_with_known_gaps` 会被归一到这 5 个主 run status；细节保存在 `raw_status`、`status_detail`、`failure_type`、`known_gaps` 和 `structured` 中，避免主状态膨胀。
...
- Execution Policy 会根据任务难度选择 inline、plan-only、reviewed plan 和 full QA 链路。
- 计划和实现之间可以有人工确认；快速修复和小型 UI 行为修复可以跳过 plan-only，直接进入受控 implementation。

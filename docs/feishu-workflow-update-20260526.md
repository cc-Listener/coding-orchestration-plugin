...
Hermes 负责流程，Runner 负责执行，人负责关键判断。

用户在飞书里提需求。Hermes 判断这是不是一个编码任务，补齐项目和上下文，生成可执行任务包，再交给 Codex 这类 Runner 去计划、实现、测试和修复。Runner 每次执行后的日志、diff、测试结果和报告都会回到 Hermes，任务状态写入 Task Ledger，有复用价值的经验写入 LLM Wiki。

| 角色 | 做什么 | 不做什么 |
| --- | --- | --- |
| 飞书 | 提需求、补反馈、确认关键节点 | 不直接驱动代码执行 |
| Hermes | 判断意图、建 task、管状态、发执行包、收证据 | 不亲自写业务代码 |
...
Coding Mode 只是降低输入成本。用户可以说“截图里的样式不对，修一下”，LLM 可以把它翻译成候选动作，比如 `/coding bugfix ...`；但是否执行，仍由 Hermes 校验。
...
4. **交给 Runner**：Hermes 生成执行包，Runner 先出计划；计划确认后才实现。
5. **验证和合入**：Runner 在受控工作区实现、测试、QA；人触发 merge-test。
6. **回收和沉淀**：Hermes 回收日志、报告、diff、测试证据，更新状态，把可复用经验写入 LLM Wiki。
...
plan-only 阶段主要读上下文，不改项目文件；implementation、QA、merge-test 在 task worktree 中执行。这样 Runner 有足够能力完成任务，但不能自己决定项目、状态、合入或发布。

每次执行结束后，Hermes 回收 `stdout.log`、`stderr.log`、`report.json`、`summary.md` 和 `diff.patch`。如果 Runner 超时、崩溃或报告格式不对，Hermes 会生成兜底报告，把状态归一到可恢复的失败或阻塞状态。
...
- Coding Mode 能把自然语言改写成标准动作，但高风险或低置信度动作必须确认。
- 计划和实现之间有人工确认。
- 实现后能进入测试和 QA，问题反馈能回到同一个 task 继续修。

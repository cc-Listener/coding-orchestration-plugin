...
| Task 30. Run orchestration service 闭环 | 9 / 17 | Run application service | In Progress | 已新增 `run_orchestration_service.py`、`run_background_orchestration.py`、`run_failure_report_projection.py`、`run_report_refinement_projection.py`、`run_diff_guard_service.py`、`run_dispatch_service.py`、`run_status_transition_service.py`、`run_start_selection_projection.py`、`run_session_projection.py`、`run_session_writeback_service.py`、`run_prompt_projection.py`、`run_context_artifact_service.py`、`run_start_artifact_service.py`、`run_manifest_artifact_service.py`、`run_stderr_artifact_service.py`、`run_report_artifact_service.py`、`run_summary_artifact_service.py`、`run_ledger_projection.py`、`run_ledger_writeback_service.py`、`run_summary_projection.py`、`run_summary_writeback_service.py`、`run_artifact_paths.py` 和 `run_project_writeback_service.py`；已迁出后台 orchestration、failure/refinement/start/session/prompt projection、run diff guard observation、runner dispatch、run status transition、run artifact 文件写入、run report/summary artifact 读写、run report summary excerpt、execution policy artifact 读取、run artifact path contract、completion/report/project writeback payload、Project writeback host gate/callback、summary writer host callback、start_run 与 active run reconcile 的 artifact / agent_run / merge-test record 写回 payload 聚合、run ledger host callback、run session host callback，以及 completed run 与 active run reconcile 的 run summary writer payload 聚合等规则。`run_context_artifact_service.py` 负责 context artifact 写入和 `execution-policy.json` 读取，`run_diff_guard_service.py` 负责 before snapshot、changed files 观测、QA report artifact 过滤、policy violations 组合和 diff summary 写回，`run_dispatch_service.py` 负责 checkpoint failure gate、`runner.run()` 调度和 runner exception fallback，`run_status_transition_service.py` 负责状态机、status/phase/Kanban callback、run start/completion/reconcile transition 和 active run cleanup，`run_artifact_paths.py` 只返回 `ArtifactSet` 路径，`run_report_artifact_service.py` 只读写指定 `report.json` 并读取 `summary_markdown` excerpt，`run_summary_artifact_service.py` 只读写指定 `summary.md`，`run_project_writeback_service.py` 只处理 Project writeback stale gate 和注入 callback 调用，`run_summary_writeback_service.py` 只处理 summary projection 到注入 writer callback 的 host 调用，`run_ledger_projection.py` 只返回 ledger writeback records，`run_ledger_writeback_service.py` 只消费 records 并调用注入 ledger callback，`run_session_writeback_service.py` 只消费 session update dict 并调用注入 task session callback，`run_summary_projection.py` 只返回 summary writeback payload；QA evidence、implementation dirty-check 和 WorkItemService 业务写回仍留在 orchestrator host/WorkItemService 边界。`orchestrator.py` 当前 4766 行，`run_status_transition_service.py` 当前 167 行，`run_dispatch_service.py` 当前 46 行，`run_orchestration_service.py` 当前 419 行，background notifier 继续只做 host 通知 |
...
| Task 30 | Run orchestration service 闭环 | In Progress | 已新增 `coding_orchestration/run_orchestration_service.py`、`run_background_orchestration.py`、`run_failure_report_projection.py`、`run_report_refinement_projection.py`、`run_diff_guard_service.py`、`run_dispatch_service.py`、`run_status_transition_service.py`、`run_start_selection_projection.py`、`run_session_projection.py`、`run_session_writeback_service.py`、`run_prompt_projection.py`、`run_context_artifact_service.py`、`run_start_artifact_service.py`、`run_manifest_artifact_service.py`、`run_stderr_artifact_service.py`、`run_report_artifact_service.py`、`run_summary_artifact_service.py`、`run_ledger_projection.py`、`run_ledger_writeback_service.py`、`run_summary_projection.py`、`run_summary_writeback_service.py`、`run_artifact_paths.py`、`run_project_writeback_service.py` 及对应 contract tests；后台 orchestration、failure/refinement/start/session/prompt projection、run diff guard observation、runner dispatch、run status transition、run artifact 文件写入、run report/summary artifact 读写、run report summary excerpt、execution policy artifact 读取、run artifact path contract、completion/report/project writeback payload、Project writeback host gate/callback、summary writer host callback、start_run 与 active run reconcile 的 artifact / agent_run / merge-test record 写回 payload 聚合、run ledger host callback、run session host callback、completed run 与 active run reconcile 的 run summary writer payload 聚合已迁出 orchestrator。`run_context_artifact_service.py` 负责 context artifact 写入和 `execution-policy.json` 读取，`run_diff_guard_service.py` 负责 before snapshot、changed files 观测、QA report artifact 过滤、policy violations 组合和 diff summary 写回，`run_dispatch_service.py` 负责 checkpoint failure gate、`runner.run()` 调度和 runner exception fallback，`run_status_transition_service.py` 负责状态机、status/phase/Kanban callback、run start/completion/reconcile transition 和 active run cleanup，`run_artifact_paths.py` 只构造 `ArtifactSet` 路径，`run_report_artifact_service.py` 只读写指定 `report.json` 并读取 `summary_markdown` excerpt，`run_summary_artifact_service.py` 只读写指定 `summary.md`，`run_project_writeback_service.py` 只处理 Project writeback stale gate 和注入 callback 调用，`run_summary_writeback_service.py` 只处理 summary projection 到注入 writer callback 的 host 调用，`run_ledger_projection.py` 只构造 ledger writeback records，`run_ledger_writeback_service.py` 只消费 records 并调用注入 ledger callback，`run_session_writeback_service.py` 只消费 session update dict 并调用注入 task session callback，`run_summary_projection.py` 只构造 LLM Wiki run summary writer payload；QA evidence、implementation dirty-check 和 WorkItemService 业务写回继续留在 orchestrator host / WorkItemService 边界。当前行数：`orchestrator.py` 4766 行，`run_status_transition_service.py` 167 行，`run_context_artifact_service.py` 136 行，`run_diff_guard_service.py` 46 行，`run_dispatch_service.py` 46 行，`run_orchestration_service.py` 419 行，`run_project_writeback_service.py` 34 行，`run_ledger_writeback_service.py` 32 行，`run_session_writeback_service.py` 16 行，`run_summary_writeback_service.py` 57 行，`run_report_artifact_service.py` 35 行，`run_summary_artifact_service.py` 17 行，`run_ledger_projection.py` 103 行，`run_summary_projection.py` 65 行，`run_artifact_paths.py` 52 行 |

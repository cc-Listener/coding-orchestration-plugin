...
| Task 34. Orchestrator façade 降载 | 6 / 17 | Architecture guard | In Progress | 已新增 `gateway_project_context.py`、`merge_test_readiness_service.py`、`project_profile_catalog.py`、`gateway_rewrite_context.py` 和 `delivery_command_executor.py`，迁出本地项目解析、blocked merge-test readiness、project profile catalog、Coding Mode rewrite context 投影、task next-step hint 纯规则以及 `/coding approve-breakdown`、`/coding materialize`、`/coding run <parent> --next`、`/coding status <parent> --delivery/--tree`、`/coding breakdown` 和 `/coding analyze` host shell / ledger / rollup / decomposition callback 绑定；`orchestrator.py` 当前 4288 行。delivery command façade 已迁完，后续继续选择非 delivery 的 façade 降载点，第一目标低于 3000 行 |

...
| Task 34. Orchestrator façade 降载 | 6 / 17 | Architecture guard | In Progress | 已新增 `gateway_project_context.py`、`merge_test_readiness_service.py`、`project_profile_catalog.py`、`gateway_rewrite_context.py`、`delivery_command_executor.py` 和 `project_command_executor.py`，迁出本地项目解析、blocked merge-test readiness、project profile catalog、Coding Mode rewrite context 投影、task next-step hint 纯规则、delivery command façade，以及 `/coding project list/init/use/status/clear` 命令模式提示和 Gateway immediate project context host shell；`orchestrator.py` 当前 4212 行。后续继续选择 diagnostics、ordinary status/run presentation 或其他非 delivery façade 降载点，第一目标低于 3000 行 |

...
| Task 30. Run orchestration service 闭环 | 9 / 17 | Run application service | In Progress | 已新增 `run_orchestration_service.py`、`run_background_orchestration.py`、`run_failure_report_projection.py`、`run_report_refinement_projection.py`、`run_start_selection_projection.py`、`run_session_projection.py`、`run_prompt_projection.py`、`run_context_artifact_service.py`、`run_start_artifact_service.py`、`run_manifest_artifact_service.py`、`run_stderr_artifact_service.py`、`run_report_artifact_service.py`、`run_summary_artifact_service.py`、`run_ledger_projection.py`、`run_summary_projection.py` 和 `run_artifact_paths.py`；已迁出后台 orchestration、failure/refinement/start/session/prompt projection、run artifact 文件写入、run report/summary artifact 读写、run report summary excerpt、execution policy artifact 读取、run artifact path contract、completion/report/project writeback payload、start_run 与 active run reconcile 的 artifact / agent_run / merge-test record 写回 payload 聚合，以及 completed run 与 active run reconcile 的 run summary writer payload 聚合等规则。`run_context_artifact_service.py` 负责 context artifact 写入和 `execution-policy.json` 读取，`run_artifact_paths.py` 只返回 `ArtifactSet` 路径，`run_report_artifact_service.py` 只读写指定 `report.json` 并读取 `summary_markdown` excerpt，`run_summary_artifact_service.py` 只读写指定 `summary.md`，`run_ledger_projection.py` 和 `run_summary_projection.py` 只返回 writeback payload；实际 ledger append/upsert、summary writer 调用、runner 调度、diff guard、状态 transition 和 project writeback 仍留在 orchestrator host 边界。`orchestrator.py` 当前 4788 行，`run_orchestration_service.py` 当前 419 行，background notifier 继续只做 host 通知 |
...
| Task 30 | Run orchestration service 闭环 | In Progress | 已新增 `coding_orchestration/run_orchestration_service.py`、`run_background_orchestration.py`、`run_failure_report_projection.py`、`run_report_refinement_projection.py`、`run_start_selection_projection.py`、`run_session_projection.py`、`run_prompt_projection.py`、`run_context_artifact_service.py`、`run_start_artifact_service.py`、`run_manifest_artifact_service.py`、`run_stderr_artifact_service.py`、`run_report_artifact_service.py`、`run_summary_artifact_service.py`、`run_ledger_projection.py`、`run_summary_projection.py`、`run_artifact_paths.py` 及对应 contract tests；后台 orchestration、failure/refinement/start/session/prompt projection、run artifact 文件写入、run report/summary artifact 读写、run report summary excerpt、execution policy artifact 读取、run artifact path contract、completion/report/project writeback payload、start_run 与 active run reconcile 的 artifact / agent_run / merge-test record 写回 payload 聚合、completed run 与 active run reconcile 的 run summary writer payload 聚合已迁出 orchestrator。`run_context_artifact_service.py` 负责 context artifact 写入和 `execution-policy.json` 读取，`run_artifact_paths.py` 只构造 `ArtifactSet` 路径，`run_report_artifact_service.py` 只读写指定 `report.json` 并读取 `summary_markdown` excerpt，`run_summary_artifact_service.py` 只读写指定 `summary.md`，`run_ledger_projection.py` 只构造 ledger writeback records，`run_summary_projection.py` 只构造 LLM Wiki run summary writer payload；实际 `append_*` / `upsert_*`、summary writer 调用、状态 transition、runner 调度、diff guard 和 project writeback 继续留在 orchestrator host 边界。当前行数：`orchestrator.py` 4788 行，`run_context_artifact_service.py` 136 行，`run_orchestration_service.py` 419 行，`run_report_artifact_service.py` 35 行，`run_summary_artifact_service.py` 17 行，`run_ledger_projection.py` 103 行，`run_summary_projection.py` 65 行，`run_artifact_paths.py` 52 行 |

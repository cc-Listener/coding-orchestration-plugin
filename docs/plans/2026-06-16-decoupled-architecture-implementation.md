...
| Task 31. SourcePort 消费闭环 | 13 | Source adapter + Task service | In Progress | 已新增 `source_projection.py`，把 `SourceResult` / legacy `source_context` 投影为稳定来源字段；prompt source block 已优先消费投影并保留 legacy prompt 行为。后续继续迁 TaskService、context-index、manifest 权限判断和 orchestrator enrichment，reader-specific dict 仅保留在 adapter/ledger 兼容层 |

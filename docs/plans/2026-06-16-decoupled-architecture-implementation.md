...
| Task 34. Orchestrator façade 降载 | 1 / 17 | Architecture guard | In Progress | 已新增 `gateway_project_context.py`，迁出本地项目候选、搜索根、路径解析和人工别名抽取纯规则；`orchestrator.py` 从 4701 行降到 4647 行。后续继续迁 project profile 读取/格式化、delivery command façade、merge-test readiness 和 rewrite context，第一目标低于 3000 行 |

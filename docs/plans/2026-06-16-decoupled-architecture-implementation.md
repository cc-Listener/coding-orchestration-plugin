...
| Task 34. Orchestrator façade 降载 | 4 / 17 | Architecture guard | In Progress | 已新增 `gateway_project_context.py`、`merge_test_readiness_service.py`、`project_profile_catalog.py` 和 `gateway_rewrite_context.py`，迁出本地项目解析、blocked merge-test readiness、project profile catalog、Coding Mode rewrite context 投影和 task next-step hint 纯规则；`orchestrator.py` 当前 4377 行。后续继续迁 delivery command façade，第一目标低于 3000 行 |

...
| Task 31. SourcePort 消费闭环 | 13 | Source adapter + Task service | In Progress | 已新增 `source_projection.py`，把 `SourceResult` / legacy `source_context` 投影为稳定来源字段；prompt source block 已优先消费投影并保留 legacy prompt 行为；context-index 已追加稳定 `source_projection` 并保留 legacy `source_context`；ContextAssembler 当前任务块已通过 projection 输出 `source_summary`；TaskService status payload / next_actions 已消费 projection；run manifest source permission gate 已通过 projection 判断 plan-only source elevation；orchestrator run 前 deferred source enrichment 已通过 projection 判断是否刷新 Feishu source。reader-specific dict 仅保留在 adapter/ledger 兼容层；dashboard source health / diagnostic helper 可后续单独小切片收口 |
| Task 32. Tool / MCP operation dispatcher | 5 / 6 | Tool contract + WorkItem adapter | In Progress | 已新增 `tool_operation_dispatcher.py`，Hermes native tool 注册层改为 `ToolSpec.operation_id -> ToolOperationDispatcher`，不再维护 `operation_id -> CodingOrchestrator.tool_*` 方法映射；`CodingOrchestrator.tool_*` 仅保留兼容 wrapper，实际 dispatcher 优先分发到 `TaskService`、`WorkItemService` 和 source/run host action，`WorkItemService.create_task` 已改为调用 `TaskService`；CLI `lark-preflight` / `source-resolve` 已直接 dispatch `source.lark_preflight` / `source.resolve`，CLI `project-mcp-preflight` 已在 host config 与 stdio command readiness gate 后 dispatch `project.mcp_preflight`；Gateway `/coding project-mcp-preflight` 已补成 diagnostic immediate reply；后续继续评估 CLI `status <task_id>` 读取 `task.status` dispatcher 和 future MCP tool，共享 dispatcher 不应下沉 `doctor` 聚合或 `status` presentation |
| Task 33. Skill 零耦合复查 | 14 | Skill contract | Complete | core skill 不包含 Hermes、`/coding`、运行根、ledger、LLM Wiki 或本机命令；Hermes operator / health binding skill 只做 host 映射和恢复命令，不再复制通用 playbook、readiness 输出格式或内部状态规则 |
| Task 34. Orchestrator façade 降载 | 6 / 17 | Architecture guard | In Progress | 已新增 `gateway_project_context.py`、`merge_test_readiness_service.py`、`project_profile_catalog.py`、`gateway_rewrite_context.py`、`delivery_command_executor.py`、`project_command_executor.py`、`coding_diagnostics_command_executor.py` 和 `coding_status_command_executor.py`，迁出本地项目解析、blocked merge-test readiness、project profile catalog、Coding Mode rewrite context 投影、task next-step hint 纯规则、delivery command façade、`/coding project list/init/use/status/clear` 命令模式提示、Gateway immediate project context host shell、doctor / lark-preflight / project-mcp-preflight / source-resolve diagnostic host shell，以及普通 status / Gateway status host shell；`orchestrator.py` 当前 4110 行。后续继续选择 ordinary run/implement command、Gateway 执行副作用或其他非 delivery/status façade 降载点，第一目标低于 3000 行 |
...
- SourceProjection prompt source block slice：新增 `coding_orchestration/source_projection.py` 和 `tests/test_source_projection.py`，把 `SourceResult` / legacy `source_context` 投影为稳定 `SourceProjection` 字段；`prompts/source_block.py` 改为通过 projection 渲染 `source_status`、`lark_cli_command`、`recovery_action`、raw_fields 和 deferred/Codex note，仍保留 legacy `source_context` 持久化结构。验证：RED 先确认新模块缺失；source projection / prompt templates 9 tests passed；source flow / source plan / task service / context assembler / context artifact 相邻 tests 26 passed。
- SourceProjection context-index slice：`source_projection.py` 新增 `source_projection_to_dict()`，`run_context_artifact_service.write_run_context_artifacts()` 写 `context-index.json` 时追加 `source_projection`，同时保留 legacy `source_context`；该切片不改 TaskService、ledger schema、manifest 权限判断或 run 前 source enrichment。验证：RED 先确认 context-index 缺少 `source_projection`；source projection / run context artifact 10 tests passed。
- ContextAssembler source summary slice：`ContextAssembler._current_task_block()` 改为通过 `source_projection_from_source()` 读取 `SourceProjection.raw_fields_summary` 输出 `source_summary`，不再直接读取 legacy `source_context.raw_fields_summary`；该切片不输出完整 raw_fields、不改 TaskService status payload、不改 manifest 权限判断或 orchestrator enrichment。验证：RED 先确认旧实现输出 legacy summary；context assembler / source projection 7 tests passed。
- TaskService status payload slice：`TaskService.task_status_payload()` 和 `next_actions_for_task_payload()` 改为消费 `SourceProjection`，用 projection 输出 `source_status`、`source_type`、`source_url`、`source_recovery_action` 和下一步动作；完整 source dict 和 legacy context dict 保持兼容区分，避免无 `source_context` 的手动来源误判为 failed；该切片不改 task 创建、ledger schema、manifest 权限判断、orchestrator enrichment 或 source reader。验证：RED 先确认旧实现输出 legacy source；task service / task status payload / source projection 12 tests passed。
- Run manifest source permission slice：`run_manifest_service.source_requires_codex_plan_permissions()` 改为通过 `source_projection_from_source()` 判断 plan-only source elevation，保留 missing/ok 不提权、Codex 可解析或 Feishu unresolved source 提权的语义；该切片不改 Codex command builder、runner、task 创建、ledger schema、orchestrator enrichment 或 source reader。验证：RED 先确认旧实现不消费 projection；run manifest / source plan / Codex command / status reconcile 38 tests passed。
...
- Tool operation dispatcher CLI project MCP preflight slice：扩展 `coding_orchestration/cli.py`、`coding_orchestration/orchestrator.py` 和 `tests/test_coding_cli.py`，让 CLI `project-mcp-preflight` 在读取 host MCP config、检查 stdio command 可用性后直接 dispatch `project.mcp_preflight`，并继续复用 `format_project_mcp_preflight()`。该切片不迁 `doctor`、`status` 或 Gateway diagnostic，也不删除 `CodingOrchestrator.tool_*` 兼容 wrapper；Task 30 保持关闭。当前行数为 `orchestrator.py` 4698 行、`cli.py` 101 行、`tests/test_coding_cli.py` 329 行。
- Tool operation dispatcher Gateway project MCP diagnostic slice：扩展 `coding_orchestration/gateway_command_controller.py`、`coding_orchestration/orchestrator.py`、`coding_orchestration/command_catalog.py`、`tests/test_gateway_command_controller.py` 和 `tests/test_gateway_command_group_flow.py`，让 Gateway `/coding project-mcp-preflight` 作为 diagnostic immediate reply 被 coding plugin 拦截，并复用 `_format_project_mcp_preflight()`；同时补齐 help / `/commands` / rewriter context 中的命令目录。该切片不迁 `doctor` 聚合、不迁 `status` presentation、不实装 future MCP host adapter；Task 31 SourcePort 投影另行推进。验证：Gateway command/catalog flow 35 tests passed；完整单测 834 passed；当前行数为 `orchestrator.py` 4701 行、`gateway_command_controller.py` 371 行、`tests/test_gateway_command_group_flow.py` 346 行。
- Tool operation dispatcher CLI task status slice：扩展 `coding_orchestration/cli.py`、`coding_orchestration/task_status_presenter.py` 和 `tests/test_coding_cli.py`，让 CLI 普通 `status <task_id>` 直接 dispatch `task.status` 并渲染 tool payload；无 task id、Gateway status、`--tree` / `--delivery` 和 active run reconcile 继续保留在 command façade。验证：RED 先确认旧路径触发 `command_coding_cli()` 断言；CLI/status/dispatcher 相邻 tests 24 passed；当前行数为 `orchestrator.py` 4701 行、`cli.py` 109 行、`task_status_presenter.py` 163 行、`tests/test_coding_cli.py` 415 行。
- Gateway project context local resolver split：新增 `coding_orchestration/gateway_project_context.py` 和 `tests/test_gateway_project_context.py`，迁出本地项目文件夹候选抽取、候选去重、搜索根去重、路径解析和人工别名抽取。orchestrator 只保留兼容 wrapper、profile 写入和默认 fallback；helper 不写 profile、不绑定 active project、不发送 Gateway 回复、不触碰 source reader 或 runner。验证：RED 先确认新模块缺失；project context contract 4 tests passed；项目上下文/source 相邻 tests 26 passed；当前行数为 `orchestrator.py` 4647 行、`gateway_project_context.py` 104 行、`tests/test_gateway_project_context.py` 58 行。
- Blocked merge-test readiness service split：新增 `coding_orchestration/merge_test_readiness_service.py` 和 `tests/test_merge_test_readiness_service.py`，迁出 blocked task 是否可继续 merge-test 的纯评估规则。orchestrator 只收集 implementation run、merge-test workspace、source branch、Codex resume session 和 structured report；service 只返回 `mergeable/requires_acceptance/reason/impact/recovery_action/fallback_evidence`，不写 ledger、不推进状态、不触发 runner、不发送 Gateway 回复。验证：RED 先确认新模块缺失；merge-test readiness service contract 4 tests passed；merge-test readiness/blocked/QA gate/basic 相邻 flow 30 tests passed；当前行数为 `orchestrator.py` 4523 行、`merge_test_readiness_service.py` 188 行、`tests/test_merge_test_readiness_service.py` 115 行。
- Project profile catalog split：新增 `coding_orchestration/project_profile_catalog.py` 和 `tests/test_project_profile_catalog.py`，迁出 project profile 读取、registry fallback、别名/path basename 查找、动态来源计数和 project list/status 格式化。orchestrator 只保留兼容 wrapper、project init/upsert、active project binding 和 Gateway 回复副作用；catalog 不写 profile、不创建任务、不触发 runner。验证：RED 先确认新模块缺失；project profile catalog contract 4 tests passed；Gateway project/rewrite/binding 相邻 flow 37 tests passed；当前行数为 `orchestrator.py` 4423 行、`project_profile_catalog.py` 142 行、`tests/test_project_profile_catalog.py` 140 行。
- Gateway rewrite context projection split：新增 `coding_orchestration/gateway_rewrite_context.py` 和 `tests/test_gateway_rewrite_context.py`，迁出 Coding Mode rewrite context 白名单投影和 task next-step hint 纯规则。orchestrator 只采集 event media、active task/project、recent tasks、known projects、command catalog 和 allowed commands 后委托 helper；command_rewriter 调用、pending rewrite 存取、确认/取消 gate、Gateway reply、ledger 读取和 project catalog 读取仍留在 host 边界。验证：RED 先确认新模块缺失；rewrite context contract 2 tests passed；Gateway rewrite/natural language/presenter 相邻 flow 19 tests passed；当前行数为 `orchestrator.py` 4370 行、`gateway_rewrite_context.py` 118 行、`tests/test_gateway_rewrite_context.py` 100 行。

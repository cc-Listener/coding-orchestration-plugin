...
| `coding_orchestration/run_context_artifact_service.py` | 109 | Run context artifact service，承接 wiki context、confirmed plan / implementation context、assembled context、run instructions、execution policy 和 context index 写入；只写 run_dir 下 context artifact，不写 ledger、manifest、report、summary，不启动 runner、不推进状态 |
